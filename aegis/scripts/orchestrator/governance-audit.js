...
export function runGovernanceAudit() {
  ...
  const schemaDir = join(ROOT, policy.artifactSchemasDir || 'aegis/orchestrator/schemas');
  const schemaFiles = [
  ...
  const prompts = safeReadJson(join(ROOT, 'aegis/orchestrator/prompts.json')) || {};
  ...
  const highestWaveFiles = readdirSync(join(ROOT, 'docs/plans/waves'))
    .map((name) => {
  ...
  if (highestWave) {
    const master = readText(join(ROOT, 'docs/plans/MASTER_PLAN.md'));
    const pending = readText(join(ROOT, 'docs/plans/PENDING_TASKS_ONLY.md'));
    const waveIndex = readText(join(ROOT, 'docs/plans/waves/README.md'));
    for (const [label, text] of [['MASTER_PLAN.md', master], ['PENDING_TASKS_ONLY.md', pending], ['docs/plans/waves/README.md', waveIndex]]) {
  ...
  const businessDocs = readText(join(ROOT, 'docs/business_docs/README.md'));
  const assistantDocs = readText(join(ROOT, 'docs/business_docs/03_ai_assistants/README.md'));
  ...
  const coverageMatrixPath = join(ROOT, 'docs/plans/FEATURE_COVERAGE_MATRIX.md');
  if (!existsSync(coverageMatrixPath)) {
    errors.push('missing docs/plans/FEATURE_COVERAGE_MATRIX.md');
  } else {
    ...
    for (const column of requiredColumns) {
      if (!coverageMatrix.includes(column)) {
        errors.push(`docs/plans/FEATURE_COVERAGE_MATRIX.md missing column: ${column}`);
      }
    }
  }
