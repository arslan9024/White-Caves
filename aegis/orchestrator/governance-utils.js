...
export const LOGS_DIR = join(ROOT, 'logs', 'orchestrator');
...
export function buildContextManifest({ policy, sessionId, dispatchPacket, previousSnapshot, routingDecision, planPacket }) {
  ...
  const waveBundles = filesInScope.filter((file) => /plans\/waves\/WAVE_/i.test(file));
  const businessDocSections = filesInScope.filter((file) => /business_docs\//i.test(file));
