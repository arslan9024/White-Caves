...
interface LeadScoringCriteria {
  ...
  savedPropertiesCount: { weight: 0.1; max: 5 };
  searchFrequency: { weight: 0.1; max: 'daily' };
  whatsappEngagement: { weight: 0.15; max: 'replied' };
  formCompleteness: { weight: 0.1; max: 100 };

  // Demographic signals (40%)
  budgetMatch: { weight: 0.15; max: 'exact' };
  locationMatch: { weight: 0.1; max: 'priority_area' };
