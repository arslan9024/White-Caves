...
Child issue #2438 (parent #1935) implements the `financeEngineUaeCorporate.types.ts`
module named in the Section 2 module layout, fulfilling the "future" placeholder
originally listed there. It does not modify any design decision recorded in
Sections 1–9 above; those remain unchanged and this addendum is purely additive.

- **Design decisions honored as-is:** AED-only currency constraint (§3.3, enforced by
  the `UaeCorporateTaxCurrency = 'AED'` literal type plus the
  `isUaeCorporateTaxCurrency`/`assertUaeCorporateTaxCurrency` runtime guards) and the
  versioned-rate-table design (§3.2, modeled here as the `UaeCorporateTaxRateTable`
  interface and `UaeCorporateTaxRateTableVersion` alias, with the concrete registry
  left to the sibling `.rates.ts` module per §2). Flooring negative taxable income at
  zero (§3.4) is explicitly left to the calculation module — this types module
  validates only structural/sign invariants of the raw input, not derived values.
- **Module boundary:** `financeEngineUaeCorporate.types.ts` contains only type
  aliases/interfaces, the `UaeCorporateTaxValidationError` class, and small runtime
  type guards/assertions. It performs no I/O and computes no tax figures (SRS FR-7),
  keeping it independently reusable by both the calculation module and any future
  API/DTO validation layer without pulling in calculation logic.
- **Testing strategy executed:** `financeEngineUaeCorporate.types.test.ts` uses vitest
  with real behavioral assertions (no placeholder assertions), covering acceptance and
  rejection paths for every exported guard/assertion, including boundary/malformed
  rate table shapes and non-mutation of validated input.
- **Completion evidence:** implementation and test files exist at the paths above;
  validation commands (`vitest run`, `typecheck`, `lint`) must be executed from the
  repository root as part of the implementation PR, since this documentation sandbox
  has no `node_modules`/`package.json` to run them against.
- **Scope confirmation:** the calculation implementation, rate table registry, and any
  integration into reporting jobs/dashboards/persistence (§4) remain explicitly
  deferred and are untouched by this addendum. No parent issue closure, bulk GitHub
  mutation, destructive database operation, or secret rewrite was performed.
- **Rollback (implementation):** revert `financeEngineUaeCorporate.types.ts`/
  `financeEngineUaeCorporate.types.test.ts` and this addendum section; the design
  documentation in Sections 1–9 stands independently and requires no rollback itself.
