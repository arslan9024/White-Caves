...
## 9. Types Extraction Addendum (Issue #2420)

- **Child issue**: #2420 (implementation child of parent #1939, sibling of
  #2421 and #2422).
- The FX data contract (`FxAmount`, `FxGainResult`) and a new `FxRate`
  alias were extracted/re-declared into a dedicated
  `src/features/finance/financeEngineFxGain/financeEngineFxGain.types.ts`
  module, alongside pure runtime guard/assertion helpers:
  `isFxAmount`, `isValidFxRate`, `assertValidFxRate`, `isSameCurrency`.
  These helpers directly implement the domain-validation semantics of
  FR-5 (a rate of `0`, `NaN`, `Infinity`, or a negative number is invalid
  and throws `RangeError`) and support FR-6 (same-currency detection) as
  reusable, independently testable primitives.
- Focused vitest coverage was added in
  `financeEngineFxGain.types.test.ts` (22 assertions) verifying the shape
  guard, rate validation guard/assertion (including the `RangeError`
  message and label), and currency-equality helper — all with real
  behavioral assertions, no placeholders.
- Scope remained limited to the declared feature folder; no unrelated
  modules were touched, no dependencies were added, and no GitHub issues
  (including parent #1939) were closed or mutated.
- See `SDD-ISSUE-W56-FINANCE-FX-1939.md` §12/§13 for completion evidence
  and the rollback note covering this addition.
