...
> Note (#2420): the type/interface/error contract (`FxTransactionInput`,
> `FxGainLossDirection`, `FxGainLossResult`, `FxGainLossSummary`,
> `FxGainErrorCode`, `FxGainCalculationError`) is defined in
> `financeEngineFxGain.types.ts` and tested in
> `financeEngineFxGain.types.test.ts`, split out from both the calculation
> logic (#2421) and the contract/README documentation (#2422) so the
> type surface can be reviewed and reverted independently of either. This
> issue does not close parent issue #1939.
...
- Type contract (#2420): `src/features/finance/financeEngineFxGain/financeEngineFxGain.types.ts`
- Type contract tests (#2420): `src/features/finance/financeEngineFxGain/financeEngineFxGain.types.test.ts`
...
other file in the module or this shared SRS/SDD pair. For issue #2420
specifically, reverting only `financeEngineFxGain.types.ts` and
`financeEngineFxGain.types.test.ts` is sufficient; no other file in this
module or repository imports from those two files at this time.
...
## 10. Completion Evidence (#2420)

Focused vitest run of `financeEngineFxGain.types.test.ts` covering: every
`FxGainErrorCode` variant constructing a distinct `FxGainCalculationError`
with the correct `message`/`code`, `instanceof Error` and `instanceof
FxGainCalculationError` prototype-chain checks surviving a throw/catch
round-trip, the error's `name` property, and object-literal shape checks
for `FxTransactionInput`, `FxGainLossResult` (across all three
`FxGainLossDirection` literals), and `FxGainLossSummary`. Evidence
(pass/fail counts) is recorded alongside the implementation commit for
#2420. Parent issue #1939 remains open pending reconciliation of all
sibling child issues under Workstream W56.
