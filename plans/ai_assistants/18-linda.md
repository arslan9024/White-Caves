...
> **Status:** ✅ In Code — `src/pages/owner/` components; backend stub  
...
|---|---|---|
| GET | `/api/linda/sessions` | List all agent sessions and their health status |
| POST | `/api/linda/sessions` | Initialize new agent session (returns QR code) |
| DELETE | `/api/linda/sessions/:agentId` | Disconnect agent session |
| GET | `/api/linda/sessions/:agentId/qr` | Get QR code for session setup |
| GET | `/api/linda/messages/:agentId` | Get mirrored messages for an agent |
| POST | `/api/linda/send` | Send message via agent's session |
| GET | `/api/linda/analytics` | Session analytics across all agents |
...
|---|---|---|
| Linda dashboard | `src/pages/owner/` WhatsApp sections | ✅ Exists (partial) |
| Session status panel | QR code + health indicators | ✅ Exists (mock) |
...
| Service | Path | Status |
|---|---|---|
| Linda route | `server/routes/linda.ts` | ✅ Exists (partial) |
| WhatsApp client manager | `server/services/LindaSessionService.ts` | 🔲 Planned |
| Session storage | `./whatsapp_sessions/` directory | 🔲 Planned |
...
- [ ] Install `whatsapp-web.js` and `qrcode` packages
- [ ] `LindaSessionService.ts` — session init, QR generation, LocalAuth
- [ ] Session health monitor + auto-reconnect
- [ ] CRM command handler (PROPERTY, LEAD, PRICING, FINANCE)
- [ ] Message mirroring to `Conversation` model
- [ ] Multi-session management (up to 50 agents)
