...
if (process.env.NODE_ENV === 'production') {
  ...
  logger.warn(
    'firebase-sync: Firebase Admin SDK not configured — token accepted without server-side verification...'
  );
}
```
...
if (photoUrl !== undefined) {
  ...
  try {
    new URL(photoUrl);
    data.photoUrl = truncateString(photoUrl.trim(), 2000);
  } catch {
