...
import { sendMessageWithReport } from "./sendMessageWithReport.js";
export async function sendBroadToList(MNumbers, Project, newChatsOnly = true) {
    console.clear();
    let SendReport, WhatsAppBotClient, AgentForIteration, message = AfternoonMessage;
    ...
    for (const [i, num] of numbers.entries()) {
        ...
        try {
            ...
            SendReport= await sendMessageWithReport(validatedContact, WhatsAppBotClient, message);
            successes++;
            await CampaignResult(SendReport, Project, successes, skipped, fails, numbers, i);
            await RandomDelay();
        } catch (error) {
            console.log(error)
