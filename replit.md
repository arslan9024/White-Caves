...
- **DashboardShell Architecture**: The MD Dashboard (`/md/dashboard`) uses a clean two-panel layout with `DashboardShell` as the container. The left panel contains `CommandSidebar` with two selectors: (1) **Assistant Selector** showing all 24 AI assistants grouped by 10 departments with expand/collapse, favorites, and status indicators; (2) **Feature Selector** dynamically showing 4-6 features per assistant based on `assistantFeatures.js` mappings. The right panel displays spacious content rendered by `ContentRenderer` which maps assistant+feature combinations to CRM components. Feature selection syncs with Redux (`selectedFeature` in `dashboardViewSlice`) and CRM components respond to the `activeFeature` prop.
