...
- **Mary's Data Tools Suite**: Three integrated data acquisition tools accessible via "Data Tools" button in Mary's CRM header:
  - **DAMAC Asset Fetcher** (`src/components/crm/inventory/DamacAssetFetcher.jsx`): S3 URL generator for fetching property images from DAMAC bucket. Features SD/Registration number input, batch URL validation, grid/list view, multi-select download.
  - **Image Data Extractor** (`src/components/crm/inventory/ImageDataExtractor.jsx`): OCR-based tool for extracting text from uploaded images. Parses UAE phone numbers, emails, names, unit numbers, and SD numbers with editable results table.
  - **Web Data Harvester** (`src/components/crm/inventory/WebDataHarvester.jsx`): URL template iterator for scraping property data from web pages. Features `[start:end]` page syntax, configurable CSS selectors, pause/stop controls, and CSV export.
- **Environment Guard Middleware** (`server/middleware/envGuard.js`): Server-side validation utility for required deployment secrets with production/development awareness.
