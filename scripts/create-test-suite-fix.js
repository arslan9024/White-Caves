...
test('Validates scoring weights sum to 1.0', () => {
  const weights = { skills: 0.35, experience: 0.25, education: 0.15, cultural_fit: 0.15, location_match: 0.10 };
...
test('Single-pass stats accumulation is efficient', () => {
  ...
  assert(time < 100, 'Should complete in <100ms');
});
...
test('Phone formatting standardizes numbers', () => {
  const format = (p) => {
    ...
    if (c.startsWith('50') || c.startsWith('52') || c.startsWith('55')) return '+971' + c;
    if (c.length === 9 && c.startsWith('0')) return '+971' + c.slice(1);
    return '+' + c;
  };
...
test('Logger produces structured output', () => {
  const log = { timestamp: new Date().toISOString(), level: 'INFO', message: 'test' };
