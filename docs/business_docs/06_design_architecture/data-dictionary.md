...
> **Note:** RERA permit details (e.g. permit number, expiry) are **not** currently stored on the `Property` model in `prisma/schema.prisma`. When permit storage is implemented (either as new `Property` fields or via a related compliance entity), this data dictionary will be updated accordingly.
...
**Source Enum:** `whatsapp`, `website`, `phone`, `referral`, `marketing`, `direct`
