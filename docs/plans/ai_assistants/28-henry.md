...
> **Status:** ✅ Production-ready — registered in code with active `/api/henry` routes  
...
| ------ | ------------------------------- | ---------------------------- |
| GET    | `/api/henry/health`             | Service health check         |
| GET    | `/api/henry/records`            | List/search archived records |
| POST   | `/api/henry/records`            | Create record index entry    |
| GET    | `/api/henry/records/file`       | Download stored PDF          |
| POST   | `/api/henry/records/file`       | Upload/archive PDF           |
| DELETE | `/api/henry/records/:id`        | Delete record entry          |
| POST   | `/api/henry/compliance/check`   | Run compliance check         |
| GET    | `/api/henry/compliance/summary` | Compliance rule summary      |
| POST   | `/api/henry/ocr/emirates-id`    | OCR Emirates ID              |
| POST   | `/api/henry/ai/extract`         | AI field extraction          |
...
| ------------------- | ----------------------------------- | --------------------------------------------------------------- |
| Henry CRM dashboard | `src/components/owner/ai/HenryCRM/` | ✅ Available via dashboard tab                                  |
| Audit log tab       | In CRM settings                     | 🔄 In progress (route layer complete; deep timeline UX pending) |
| Entity timeline     | Embedded in lead/property detail    | 🔄 In progress                                                  |
| SLA breach panel    | Inside dashboard                    | 🔄 In progress                                                  |
...
| Service                          | Path                                                         | Status         |
| -------------------------------- | ------------------------------------------------------------ | -------------- |
| Henry route surface              | `server/routes/henry.ts`                                     | ✅ Implemented |
| Compliance engine                | `server/services/henry/complianceEngine.ts`                  | ✅ Implemented |
| OCR + AI extraction integrations | `server/routes/henry.ts` (`/ocr/emirates-id`, `/ai/extract`) | ✅ Implemented |
| SLA/timeline expansion layer     | dedicated service + cron modules                             | 🔄 In progress |
