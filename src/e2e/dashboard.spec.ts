...
test.describe('Dashboard Navigation & Layout', () => {
  ...
  test('should render dashboard shell markers when dashboard route is accessible', async ({ page }) => {
    await page.goto('/modern-dashboard', { waitUntil: 'domcontentloaded' });

    const pathname = new URL(page.url()).pathname;
    test.skip(!pathname.startsWith('/modern-dashboard'), `Dashboard not accessible in this auth state: ${pathname}`);
  ...
  test('should preserve sign-in discoverability from home', async ({ page }) => {
    ...
    test.skip(!hasSignInEntry, 'Sign-in discoverability marker not present in this homepage variant.');
