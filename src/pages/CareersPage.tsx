            ...
            <div className="positions-list">
              {jobPositions.map(job => (
                <div key={job.id} className="position-card">
                  ...
                  <button
                    className="apply-btn"
                    onClick={() => { setApplyingFor(job.title); setApplyStatus('idle'); }}
        ...
        >
          ...
          >
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
              ...
              <button onClick={closeApplyModal} style={{ background: 'none', border: 'none', fontSize: '1.5rem', cursor: 'pointer' }} aria-label="Close">×</button>
            </div>
            ...
            {applyStatus === 'success' ? (
              <div style={{ textAlign: 'center', padding: '1.5rem 0' }}>
                ...
                <p style={{ color: 'var(--text-secondary)', marginBottom: '1.5rem' }}>
                  Thank you for applying. We'll review your application and be in touch within 5 business days.
                </p>
                <button onClick={closeApplyModal} className="apply-btn" style={{ display: 'inline-block' }}>
                  Close
                </button>
              </div>
            ) : (
              <form onSubmit={handleApplySubmit}>
                {applyError && (
                  <p style={{ color: '#B91C1C', background: '#FEF2F2', padding: '0.75rem', borderRadius: '8px', marginBottom: '1rem' }}>
                    {applyError}
                  </p>
                )}
                ...
                <div style={{ marginBottom: '1.5rem' }}>
                  <label style={{ display: 'block', marginBottom: '0.25rem', fontWeight: 500 }}>Cover Letter</label>
                  <textarea
                    rows={4}
                    value={applyForm.coverLetter}
                    placeholder="Tell us why you'd be a great fit…"
                    onChange={(e) => setApplyForm((p) => ({ ...p, coverLetter: e.target.value }))}
                    style={{ width: '100%', padding: '0.6rem', border: '1px solid #ddd', borderRadius: '8px', resize: 'vertical', boxSizing: 'border-box' }}
                  />
                </div>
                <button
                  type="submit"
