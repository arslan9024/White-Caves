 ...
 * ⚠️ BRAND RULE: Red (#E31E24) + White ONLY. NO gold, yellow, amber, or tan.
...
export const COLOR_TOKENS = {
  ...
  // Accent Colors
  accent: {
    brandRed: '#E31E24', // Brand primary (formerly misnamed 'gold')
  ...
  // Gradient System
  gradients: {
    redDeep: 'linear-gradient(135deg, #C41E3A 0%, #E31E24 100%)',
    ...
    darkNeutral: 'linear-gradient(135deg, #1A1A1A 0%, #4A4A4A 100%)',
    lightNeutral: 'linear-gradient(135deg, #F9F9F9 0%, #FFFFFF 100%)',
  },
...
export const getColorByDepartment = departmentId => {
  const validDepts = Object.keys(COLOR_TOKENS.departments);
  const key = validDepts.includes(departmentId) ? departmentId : 'executive';
  // eslint-disable-next-line security/detect-object-injection
  return COLOR_TOKENS.departments[key];
};
