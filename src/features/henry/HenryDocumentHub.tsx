...
export const DOCUMENT_TEMPLATES: DocumentTemplate[] = [
  {
    ...
    fields: [
      ...
      { name: 'tenantEmiratesId', label: 'Tenant Emirates ID', type: 'text', required: false, placeholder: '784-YYYY-XXXXXXX-D' },
      { name: 'ejariAcknowledged', label: 'Ejari Registration Acknowledged', type: 'boolean', required: true },
    ],
  },
  {
    key: 'booking_form',
    ...
    fields: [
      ...
      { name: 'buyerEmiratesId', label: 'Buyer Emirates ID', type: 'text', required: false, placeholder: '784-YYYY-XXXXXXX-D' },
  ...
  },
  {
    ...
    fields: [
      ...
      { name: 'tenantName', label: 'Tenant Full Name', type: 'text', required: true },
      { name: 'propertyAddress', label: 'Property Address', type: 'text', required: true },
      { name: 'originalContractRef', label: 'Original Contract Number', type: 'text', required: true },
      { name: 'documentDate', label: 'Addendum Date', type: 'date', required: true },
      { name: 'rentIncreasePercent', label: 'Rent Increase %', type: 'number', required: false, placeholder: 'Max 20% per Decree 43/2013' },
      { name: 'leaseEndDate', label: 'Existing Lease End Date', type: 'date', required: false },
      { name: 'noticeDate', label: 'Notice Date (must be 90+ days before expiry)', type: 'date', required: false },
    ],
  },
  {
    key: 'offer_letter',
  ...
  },
  {
    ...
    fields: [
      ...
      { name: 'trnNumber', label: 'TRN Number', type: 'text', required: false, placeholder: 'Tax Registration Number' },
  ...
  },
  {
    ...
    fields: [
      ...
      { name: 'propertyAddress', label: 'Property Address', type: 'text', required: true },
      { name: 'purchasePrice', label: 'Purchase / Annual Rent (AED)', type: 'number', required: true },
      { name: 'tenantEmiratesId', label: 'Emirates ID', type: 'text', required: true, placeholder: '784-YYYY-XXXXXXX-D' },
...
const HenryDocumentHub: React.FC<HenryDocumentHubProps> = () => {
  ...
  const loadRecords = useCallback(async () => {
    ...
    try {
      ...
      const json = await res.json() as { success: boolean; data: { records: HenryRecord[] }; error?: string };
      if (json.success) {
  ...
  const handleCheckCompliance = async () => {
    ...
    try {
      ...
      const json = await res.json() as { success: boolean; data: ComplianceReport; error?: string };
      if (json.success) {
  ...
  const handleExtractFields = async (text: string) => {
    ...
    try {
      ...
      const json = await res.json() as { success: boolean; data: { fields: Record<string, unknown>; confidence: number }; error?: string };
      if (json.success && json.data.fields) {
        setFormData(prev => ({ ...prev, ...json.data.fields }));
        setSuccessMessage(`AI extracted fields with ${Math.round((json.data.confidence ?? 0) * 100)}% confidence`);
      } else {
  ...
  return (
    <div className="henry-document-hub" style={{ display: 'flex', height: '100%', fontFamily: 'system-ui, sans-serif' }}>
      {/* ── Left: Template List ── */}
      <aside style={{ width: 220, borderRight: '1px solid #2a2a2a', overflowY: 'auto', background: '#111', padding: '12px 0' }}>
        <div style={{ padding: '8px 16px 16px', color: '#C9A84C', fontWeight: 700, fontSize: 13, letterSpacing: 1 }}>
          📄 HENRY — DOCUMENT HUB
        </div>
        {DOCUMENT_TEMPLATES.map(t => (
          <button
            ...
            style={{
              ...
              border: 'none', cursor: 'pointer', color: activeTemplate?.key === t.key ? '#C9A84C' : '#ccc',
              borderLeft: activeTemplate?.key === t.key ? '3px solid #C9A84C' : '3px solid transparent',
              fontSize: 13,
            }}
          >
        ...
        <button
          ...
          style={{
            ...
            border: 'none', cursor: 'pointer', color: '#aaa', fontSize: 13,
          }}
        >
      ...
      <main style={{ flex: 1, overflowY: 'auto', background: '#0f0f0f', padding: 24 }}>
              ...
              {(['editor', 'compliance'] as const).map(panel => (
                <button
                  ...
                  style={{
                    ...
                    fontWeight: activePanel === panel ? 700 : 400, fontSize: 13,
                  }}
                >
        ...
        {successMessage && (
          <div style={{ background: '#1a3a1a', border: '1px solid #2e7d32', borderRadius: 6, padding: '10px 14px', marginBottom: 16, color: '#8f8', fontSize: 13 }}>
            ✅ {successMessage}
          </div>
        )}
        ...
        {!activeTemplate && activePanel === 'editor' && (
          <div style={{ textAlign: 'center', padding: '60px 0', color: '#555' }}>
            ...
            <div style={{ fontSize: 16, color: '#888' }}>Select a document template from the left panel</div>
...
const DocumentEditor: React.FC<DocumentEditorProps> = ({
  ...
  return (
    <div>
      {/* AI Extraction Panel */}
      <div style={{ background: '#1a1a2e', border: '1px solid #2a2a4a', borderRadius: 8, padding: 16, marginBottom: 20 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: showPaste ? 12 : 0 }}>
          <span style={{ color: '#9b8ff5', fontWeight: 600, fontSize: 13 }}>🤖 AI Field Extraction</span>
          <button
            onClick={() => setShowPaste(p => !p)}
            style={{ padding: '4px 12px', background: '#2a2a4a', border: '1px solid #4a4a8a', borderRadius: 4, color: '#9b8ff5', cursor: 'pointer', fontSize: 12 }}
          >
        ...
        {showPaste && (
          <div>
            <textarea
              ...
              style={{ width: '100%', background: '#111', border: '1px solid #333', borderRadius: 4, color: '#fff', padding: 10, fontSize: 13, resize: 'vertical', boxSizing: 'border-box' }}
            />
            <button
              onClick={() => { onExtractFields(pasteText); setShowPaste(false); }}
              disabled={!pasteText.trim() || isExtracting}
              style={{ marginTop: 8, padding: '6px 16px', background: '#7C3AED', border: 'none', borderRadius: 4, color: '#fff', cursor: 'pointer', fontSize: 13, opacity: (!pasteText.trim() || isExtracting) ? 0.5 : 1 }}
            >
        ...
        {template.fields.map(field => (
          <div key={field.name}>
            <label style={{ display: 'block', color: '#aaa', fontSize: 12, marginBottom: 4 }}>
              {field.label}{field.required && <span style={{ color: '#E31E24' }}> *</span>}
            </label>
            {field.type === 'textarea' ? (
              <textarea
                ...
                rows={3}
                style={{ width: '100%', background: '#1a1a1a', border: '1px solid #333', borderRadius: 4, color: '#fff', padding: '8px 10px', fontSize: 13, resize: 'vertical', boxSizing: 'border-box' }}
              />
            ) : field.type === 'select' ? (
              <select
                value={String(formData[field.name] ?? '')}
                onChange={e => onFieldChange(field.name, e.target.value)}
                style={{ width: '100%', background: '#1a1a1a', border: '1px solid #333', borderRadius: 4, color: '#fff', padding: '8px 10px', fontSize: 13 }}
              >
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 4 }}>
                ...
                />
                <label htmlFor={field.name} style={{ color: '#ccc', fontSize: 13, cursor: 'pointer' }}>Yes</label>
              </div>
            ) : (
              <input
      ...
      <div style={{ display: 'flex', gap: 12, marginTop: 16 }}>
        <button
          ...
          disabled={isChecking}
          style={{ padding: '10px 20px', background: '#1a3a1a', border: '1px solid #2e7d32', borderRadius: 6, color: '#8f8', cursor: 'pointer', fontSize: 14, opacity: isChecking ? 0.6 : 1 }}
        >
        ...
        <button
          ...
          disabled={isSaving}
          style={{ padding: '10px 20px', background: '#C9A84C', border: 'none', borderRadius: 6, color: '#000', cursor: 'pointer', fontSize: 14, fontWeight: 600, opacity: isSaving ? 0.6 : 1 }}
        >
...
const CompliancePanel: React.FC<CompliancePanelProps> = ({ report }) => {
  const severityColor = { error: '#E31E24', warning: '#f59e0b', info: '#3b82f6' };
    ...
    <div>
      ...
      <div style={{ display: 'flex', gap: 16, marginBottom: 24 }}>
        {[
          ...
          { label: 'Errors', value: report.errorCount, color: '#E31E24' },
          { label: 'Warnings', value: report.warningCount, color: '#f59e0b' },
          { label: 'Total', value: report.totalRules, color: '#aaa' },
        ].map(stat => (
          <div key={stat.label} style={{ background: '#1a1a1a', borderRadius: 8, padding: '12px 20px', textAlign: 'center', flex: 1 }}>
      ...
      <div style={{
        ...
        border: `1px solid ${report.isCompliant ? '#2e7d32' : '#E31E24'}`,
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
        {report.results.filter(r => !r.passed).map(result => (
          <div key={result.ruleId} style={{
            background: '#1a1a1a', border: `1px solid ${severityColor[result.severity]}30`,
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
              <div>
                ...
                {result.uaeLawReference && (
                  <span style={{ marginLeft: 8, color: '#666', fontSize: 11 }}>({result.uaeLawReference})</span>
                )}
              </div>
              <span style={{ color: '#666', fontSize: 11, whiteSpace: 'nowrap', marginLeft: 8 }}>{result.ruleId}</span>
            </div>
            <p style={{ color: '#aaa', margin: '6px 0 0 24px', fontSize: 12 }}>{result.message}</p>
          </div>
        ))}
        {report.results.filter(r => r.passed).map(result => (
          <div key={result.ruleId} style={{
            background: '#111', border: '1px solid #1a3a1a',
            borderLeft: '3px solid #2e7d32', borderRadius: 6, padding: '8px 14px', opacity: 0.7,
          }}>
...
const ArchivePanel: React.FC<ArchivePanelProps> = ({
  ...
  <div>
    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
      ...
      <button
        onClick={onRefresh}
        style={{ padding: '6px 14px', background: '#222', border: '1px solid #444', borderRadius: 4, color: '#aaa', cursor: 'pointer', fontSize: 12 }}
      >
        🔄 Refresh
      </button>
    </div>
    ...
    {isLoading ? (
      ...
      <div style={{ color: '#555', textAlign: 'center', padding: 40 }}>No archived documents yet. Generate and save a document to start the archive.</div>
    ) : (
