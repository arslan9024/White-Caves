...
export const fetchTenancyDeals = createAsyncThunk(
  ...
  async (params = {}, { rejectWithValue }) => {
    try {
      ...
      const response = await authFetch(`/api/deals/tenancy?${queryParams}`);
...
export const fetchSalesDeals = createAsyncThunk(
  ...
  async (params = {}, { rejectWithValue }) => {
    try {
      const queryParams = new URLSearchParams(params);
      const response = await authFetch(`/api/deals/sales?${queryParams}`);
...
export const fetchDealByNumber = createAsyncThunk(
  ...
  async ({ dealNumber, dealType }, { rejectWithValue }) => {
    try {
      ...
      const response = await authFetch(`/api/deals/${endpoint}/${dealNumber}`);
...
export const updateDealStatus = createAsyncThunk(
  ...
  async ({ dealNumber, dealType, status, notes, actor }, { rejectWithValue }) => {
    try {
      const endpoint = dealType === 'tenancy' ? 'tenancy' : 'sales';
      const response = await authFetch(`/api/deals/${endpoint}/${dealNumber}/status`, {
        method: 'PUT',
...
export const seedDemoData = createAsyncThunk('deals/seedDemo', async (_, { rejectWithValue }) => {
  try {
    const response = await authFetch('/api/deals/demo/seed', { method: 'POST' });
...
export const fetchDealStats = createAsyncThunk(
  ...
  async (_, { rejectWithValue }) => {
    try {
      const response = await authFetch('/api/deals/stats');
