// Score breakdown interface
interface ScoreBreakdown {
  expertise: number;
  availability: number;
  performance: number;
  proximity: number;
  clientMatch: number;
}

// Recommendation interface
interface Recommendation {
  level: string;
  label: string;
  color: string;
}

// Agent score result interface
interface AgentScoreResult {
  agentId: unknown;
  name: string;
  email: string;
  phone: string;
  totalScore: number;
  breakdown: ScoreBreakdown;
  recommendation: Recommendation;
}

// Weights interface
interface WeightsType {
  expertise: number;
  availability: number;
  performance: number;
  proximity: number;
  clientMatch: number;
}

// Agent interface
interface Agent {
  _id?: unknown;
  id?: unknown;
  name: string;
  email: string;
  phone: string;
  expertise?: {
    propertyTypes?: string[];
    locations?: string[];
    priceRange?: {
      min?: number;
      max?: number;
    };
  };
  activeDeals?: number;
  currentListings?: number;
  maxCapacity?: number;
  isOnLeave?: boolean;
  performance?: {
    closingRate?: number;
    avgDaysToClose?: number;
    clientRating?: number;
  };
  location?: {
    area?: string;
    emirate?: string;
  };
  primaryArea?: string;
  emirate?: string;
  languages?: string[];
  specializations?: {
    nationalities?: string[];
  };
}

// Property interface
interface Property {
  specifications?: {
    propertyType?: string;
  };
  type?: string;
  location?: {
    emirate?: string;
    community?: string;
  };
  emirate?: string;
  community?: string;
  pricing?: {
    amount?: number;
  };
  price?: number;
}

// Client interface
interface Client {
  preferredLanguage?: string;
  nationality?: string;
  budget?: number;
}

class AgentAssignmentEngine {
  private weights: WeightsType;

  constructor() {
    this.weights = {
      expertise: 0.35,
      availability: 0.25,
      performance: 0.2,
      proximity: 0.1,
      clientMatch: 0.1
    };
  }

  async assignAgent(property: Property, client: Client, agents: Agent[]): Promise<AgentScoreResult[]> {
    if (!agents || agents.length === 0) {
      return [];
    }

    const scores = await Promise.all(
      agents.map(async (agent) => {
        const scoreBreakdown: ScoreBreakdown = {
          expertise: this.calculateExpertiseScore(agent, property),
          availability: this.calculateAvailabilityScore(agent),
          performance: this.calculatePerformanceScore(agent),
          proximity: this.calculateProximityScore(agent, property),
          clientMatch: this.calculateClientMatchScore(agent, client)
        };

        const totalScore = Object.keys(scoreBreakdown).reduce((sum, key) => {
          return sum + ((scoreBreakdown as Record<string, number>)[key] * (this.weights as Record<string, number>)[key]);
        }, 0);

        return {
          agentId: agent._id || agent.id,
          name: agent.name,
          email: agent.email,
          phone: agent.phone,
          totalScore: parseFloat(totalScore.toFixed(2)),
          breakdown: scoreBreakdown,
          recommendation: this.getRecommendation(totalScore)
        };
      })
    );

    return scores.sort((a, b) => b.totalScore - a.totalScore);
  }

  private calculateExpertiseScore(agent: Agent, property: Property): number {
    if (!agent || !property) return 50;

    let score = 0;
    const maxScore = 100;

    const propertyTypeExpertise = agent.expertise?.propertyTypes || [];
    const propertyType = property.specifications?.propertyType || property.type;
    if (propertyTypeExpertise.includes(propertyType as string)) {
      score += 40;
    }

    const locationExpertise = agent.expertise?.locations || [];
    const propertyLocation = property.location?.emirate || property.emirate;
    if (locationExpertise.includes(propertyLocation as string)) {
      score += 35;
    }

    const priceExpertise = agent.expertise?.priceRange || {};
    const propertyPrice = property.pricing?.amount || property.price;
    if (
      propertyPrice !== undefined &&
      propertyPrice >= (priceExpertise.min || 0) &&
      propertyPrice <= (priceExpertise.max || Infinity)
    ) {
      score += 25;
    }

    return Math.min(score, maxScore);
  }

  private calculateAvailabilityScore(agent: Agent): number {
    if (!agent) return 50;

    let score = 100;
    const activeDeals = agent.activeDeals || agent.currentListings || 0;
    const maxDeals = agent.maxCapacity || 10;

    const utilizationRate = activeDeals / maxDeals;
    if (utilizationRate > 0.9) {
      score = 20;
    } else if (utilizationRate > 0.7) {
      score = 50;
    } else if (utilizationRate > 0.5) {
      score = 70;
    } else {
      score = 100;
    }

    if (agent.isOnLeave) {
      score = 0;
    }

    return score;
  }

  private calculatePerformanceScore(agent: Agent): number {
    if (!agent) return 50;

    let score = 0;
    const performance = agent.performance || {};

    const closingRate = performance.closingRate || 0;
    score += Math.min(closingRate * 0.4, 40);

    const avgDays = performance.avgDaysToClose || 60;
    if (avgDays < 30) {
      score += 30;
    } else if (avgDays < 45) {
      score += 20;
    } else if (avgDays < 60) {
      score += 10;
    }

    const rating = performance.clientRating || 0;
    score += (rating / 5) * 30;

    return Math.min(score, 100);
  }

  private calculateProximityScore(agent: Agent, property: Property): number {
    if (!agent || !property) return 50;

    const agentLocation = agent.location?.area || agent.primaryArea;
    const propertyLocation = property.location?.community || property.community;

    if (!agentLocation || !propertyLocation) return 50;

    if (agentLocation === propertyLocation) {
      return 100;
    }

    const agentEmirate = agent.location?.emirate || agent.emirate;
    const propertyEmirate = property.location?.emirate || property.emirate;
    if (agentEmirate === propertyEmirate) {
      return 70;
    }

    return 30;
  }

  private calculateClientMatchScore(agent: Agent, client: Client): number {
    if (!agent || !client) return 50;

    let score = 0;

    if (client.preferredLanguage) {
      const agentLanguages = agent.languages || ['en'];
      if (agentLanguages.includes(client.preferredLanguage)) {
        score += 40;
      }
    } else {
      score += 20;
    }

    if (client.nationality && agent.specializations?.nationalities) {
      if (agent.specializations.nationalities.includes(client.nationality)) {
        score += 30;
      }
    } else {
      score += 15;
    }

    if (client.budget) {
      const agentPriceRange = agent.expertise?.priceRange || {};
      if (
        client.budget >= (agentPriceRange.min || 0) &&
        client.budget <= (agentPriceRange.max || Infinity)
      ) {
        score += 30;
      }
    } else {
      score += 15;
    }

    return Math.min(score, 100);
  }

  private getRecommendation(score: number): Recommendation {
    if (score >= 80) {
      return {
        level: 'highly_recommended',
        label: 'Highly Recommended',
        color: '#22C55E'
      };
    } else if (score >= 60) {
      return {
        level: 'recommended',
        label: 'Recommended',
        color: '#3B82F6'
      };
    } else if (score >= 40) {
      return {
        level: 'suitable',
        label: 'Suitable',
        color: '#F59E0B'
      };
    } else {
      return {
        level: 'not_recommended',
        label: 'Not Recommended',
        color: '#EF4444'
      };
    }
  }

  async getBestAgent(
    property: Property,
    client: Client,
    agents: Agent[]
  ): Promise<AgentScoreResult | null> {
    const rankedAgents = await this.assignAgent(property, client, agents);
    return rankedAgents.length > 0 ? rankedAgents[0] : null;
  }

  async getTopAgents(
    property: Property,
    client: Client,
    agents: Agent[],
    count: number = 3
  ): Promise<AgentScoreResult[]> {
    const rankedAgents = await this.assignAgent(property, client, agents);
    return rankedAgents.slice(0, count);
  }

  updateWeights(newWeights: Partial<WeightsType>): void {
    const total = Object.values(newWeights).reduce((sum, w) => sum + (w || 0), 0);
    if (Math.abs(total - 1.0) > 0.01) {
      throw new Error('Weights must sum to 1.0');
    }
    this.weights = { ...this.weights, ...newWeights };
  }
}

export default AgentAssignmentEngine;
