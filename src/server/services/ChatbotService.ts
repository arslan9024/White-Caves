...
const trainingData: TrainingDataType = {
  intents: [
    ...
    },
    {
      ...
      priority: 1,
      examples: {
        en: ['bye', 'goodbye', 'see you', 'talk later', 'have a nice day', 'take care', 'good night', 'later'],
        ar: [
          'مع السلامة',
          'وداعاً',
          'إلى اللقاء',
...
class ChatbotService {
  private trainingData: TrainingDataType;
  ...
  private extractEntities(message: string, language: 'en' | 'ar'): ExtractedEntities {
    const entities: ExtractedEntities = {};
    const normalizedMessage = this.normalizeText(message, language);

    Object.entries(this.trainingData.entities.propertyType[language] || {}).forEach(
      ([type, synonyms]) => {
    ...
    Object.entries(this.trainingData.entities.location[language] || {}).forEach(
      ([loc, synonyms]) => {
        if (synonyms.some((syn) => normalizedMessage.includes(syn.toLowerCase()))) {
  ...
  private findBestIntent(message: string, language: 'en' | 'ar'): IntentMatch {
    const normalizedMessage = this.normalizeText(message, language);
  ...
  processMessage(message: string, conversationId: string = 'default'): ProcessMessageResult {
    const language = this.detectLanguage(message) as 'en' | 'ar';
    ...
    context.lastIntent = intent?.name || null;
  ...
  private getSuggestedActions(intent: Intent | null, language: 'en' | 'ar'): string[] {
    const actions: SuggestedActionsMap = {
      en: {
    ...
    if (!intent) return [];
    return actions[language]?.[intent.name as keyof typeof actions.en] || actions.en[intent.name as keyof typeof actions.en] || [];
  }
  ...
  calculateLeadScore(conversationId: string): number {
