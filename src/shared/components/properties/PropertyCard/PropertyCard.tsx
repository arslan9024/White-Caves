...
export interface PropertyCardData {
  ...
  status?: 'available' | 'sold' | 'rented' | 'pending' | 'off-market' | string;
  images?: string[];
  featured?: boolean;
  propertyType?: string;
}
...
const PropertyCard = React.memo<PropertyCardProps>(({
  ...
  const handleSave = useCallback((e: React.MouseEvent): void => {
    e.preventDefault();
    e.stopPropagation();
    onSave?.(id);
  }, [id, onSave]);

  const handleContact = useCallback((e: React.MouseEvent): void => {
    e.preventDefault();
    e.stopPropagation();
    onContact?.(property);
  }, [property, onContact]);
