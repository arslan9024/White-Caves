...
describe('propertySearchSlice', () => {
  ...
  it('setIntentProfile — sets buy', () => {
    expect(
      propertySearchReducer(getInitialState(), setIntentProfile('buy')).intentProfile,
    ).toBe('buy');
  });

  it('setIntentProfile — sets rent', () => {
    expect(
      propertySearchReducer(getInitialState(), setIntentProfile('rent')).intentProfile,
    ).toBe('rent');
  });

  it('setIntentProfile — sets invest', () => {
    expect(
      propertySearchReducer(getInitialState(), setIntentProfile('invest')).intentProfile,
    ).toBe('invest');
  });
  ...
  it('setFilters — updates permitStatus', () => {
    const state = propertySearchReducer(
      getInitialState(),
      setFilters({ permitStatus: 'active' }),
    );
