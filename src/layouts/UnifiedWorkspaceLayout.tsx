...
import FloatingSearchPill from '../components/navigation/FloatingSearchPill';
...
export const UnifiedWorkspaceLayout: React.FC<UnifiedWorkspaceLayoutProps> = ({
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', minHeight: '100vh', width: '100vw', backgroundColor: 'var(--bg-canvas, #FFFFFF)', fontFamily: 'Inter, sans-serif' }}>
      {/* ── FIXED TOP NAVBAR (Pillar 1) ────────────────────────────────────────── */}
      <TopNavbar />

      {/* ── FLOATING SEARCH COMMAND PILL (Pillar 1) ────────────────────────────── */}
      <FloatingSearchPill />

      {/* ── MAIN WORKSPACE BODY WITH PT-16 (64px) CONTENT OVERLAP PADDING ─────── */}
      ...
      <aside style={{ width: '320px', backgroundColor: 'var(--color-1e293b, #1E293B)', color: 'var(--white, #FFFFFF)', display: 'flex', flexDirection: 'column', borderRight: '1px solid var(--color-334155, #334155)', flexShrink: 0 }}>
        ...
        {userProfile.isFounder && (
          <div style={{ padding: '10px 20px', backgroundColor: 'rgba(239, 68, 68, 0.15)', borderBottom: '1px solid rgba(239, 68, 68, 0.3)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            ...
            <span style={{ fontSize: '10px', background: 'var(--accent-red, #EF4444)', color: 'var(--white, #FFF)', padding: '2px 6px', borderRadius: '4px', fontWeight: 'bold' }}>FOUNDER</span>
          </div>
        )}
        ...
        <div style={{ padding: '12px 16px', borderBottom: '1px solid var(--color-334155, #334155)' }}>
          <input
            ...
            style={{ width: '100%', padding: '8px 12px', borderRadius: '6px', border: '1px solid var(--color-475569, #475569)', backgroundColor: 'var(--color-0f172a, #0F172A)', color: 'var(--white, #FFFFFF)', fontSize: '13px', outline: 'none' }}
          />
        ...
        <div style={{ padding: '16px', backgroundColor: 'var(--color-0f172a, #0F172A)', borderTop: '1px solid var(--color-334155, #334155)', display: 'flex', alignItems: 'center', gap: '12px' }}>
          <div style={{ width: '36px', height: '36px', borderRadius: '50%', backgroundColor: 'var(--accent-red, #EF4444)', color: 'var(--white, #FFF)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 'bold' }}>
          ...
          {children || (
            <div style={{ backgroundColor: 'var(--white, #FFFFFF)', padding: '32px', borderRadius: '12px', border: '1px solid var(--text-secondary, #E2E8F0)', boxShadow: '0 1px 3px rgba(0,0,0,0.05)' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '16px', marginBottom: '32px' }}>
                <div style={{ padding: '16px', backgroundColor: 'var(--color-fff5f5, #FFF5F5)', border: '1px solid var(--color-fecaca, #FECACA)', borderRadius: '8px' }}>
                  <div style={{ fontSize: '11px', color: 'var(--accent-red, #EF4444)', fontWeight: 'bold' }}>STEP 1: ENTRY</div>
                  <div style={{ fontSize: '14px', fontWeight: '600', color: 'var(--color-1e293b, #1E293B)', marginTop: '4px' }}>{activeView.entryPoint}</div>
                </div>
                <div style={{ padding: '16px', backgroundColor: 'var(--color-f8fafc, #F8FAFC)', border: '1px solid var(--text-secondary, #E2E8F0)', borderRadius: '8px' }}>
                  <div style={{ fontSize: '11px', color: 'var(--text-secondary, #64748B)', fontWeight: 'bold' }}>STEP 2: INPUT</div>
                  <div style={{ fontSize: '14px', fontWeight: '600', color: 'var(--color-1e293b, #1E293B)', marginTop: '4px' }}>Data Validation & Context</div>
                </div>
                <div style={{ padding: '16px', backgroundColor: 'var(--color-f8fafc, #F8FAFC)', border: '1px solid var(--text-secondary, #E2E8F0)', borderRadius: '8px' }}>
                  <div style={{ fontSize: '11px', color: 'var(--text-secondary, #64748B)', fontWeight: 'bold' }}>STEP 3: PROCESSING</div>
                  <div style={{ fontSize: '14px', fontWeight: '600', color: 'var(--color-1e293b, #1E293B)', marginTop: '4px' }}>Business Logic Execution</div>
                </div>
                <div style={{ padding: '16px', backgroundColor: 'var(--color-f8fafc, #F8FAFC)', border: '1px solid var(--text-secondary, #E2E8F0)', borderRadius: '8px' }}>
                  <div style={{ fontSize: '11px', color: 'var(--text-secondary, #64748B)', fontWeight: 'bold' }}>STEP 4: CONFIRMATION</div>
                  <div style={{ fontSize: '14px', fontWeight: '600', color: 'var(--color-1e293b, #1E293B)', marginTop: '4px' }}>Task Completion State</div>
                </div>
              </div>

              {/* Live Workspace Content Placeholder */}
              <div style={{ padding: '40px', textAlign: 'center', backgroundColor: 'var(--color-f8fafc, #F8FAFC)', borderRadius: '8px', border: '2px dashed var(--text-secondary, #CBD5E1)' }}>
