...
export class AgentAssignmentEngine {
  ...
  async calculateAgentScore(agent: any, property: any, client: any = null): Promise<AgentScore> {
    const scores = {
  ...
  calculateExpertiseScore(agent: any, property: any): number {
  ...
  isInExpertiseRange(agent: any, price: number): boolean {
  ...
  async calculateAvailabilityScore(agent: any): Promise<number> {
    if (!agent) return 0;
  ...
  checkWorkingHours(agent: any): number {
    const now = new Date();
  ...
  calculatePerformanceScore(agent: any): number {
  ...
  normalizeResponseTime(avgResponseTime: number | null): number {
  ...
  calculateProximityScore(agent: any, property: any): number {
  ...
  calculateClientPreference(agent: any, client: any): number {
  ...
  generateRecommendations(scores: Record<string, number>, agent: any, property: any): any[] {
  ...
  async findBestAgent(agents: any[], property: any, client: any = null) {
  ...
  async roundRobinAssignment(agents: any[], excludeAgentIds: string[] = []) {
  ...
  calculateTeamWorkload(agents: any[]) {
    const workload = {
