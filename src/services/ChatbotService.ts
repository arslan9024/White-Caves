...
export class ChatbotService {
  ...
  detectLanguage(message: string): 'ar' | 'en' {
    const arabicPattern = /[\u0600-\u06FF]/;
    return arabicPattern.test(message) ? 'ar' : 'en';
  }

  normalizeText(text: string, language: string): string {
  ...
  extractEntities(message: string, language: string): Record<string, any> {
  ...
  findBestIntent(message: string, language: string) {
    const normalizedMessage = this.normalizeText(message, language);
    let bestMatch = null;
    let highestScore = 0;
  ...
  generateResponse(intent: any, entities: Record<string, any>, language: string, conversationId: string): string {
    if (!intent) {
  ...
  processMessage(message: string, conversationId = 'default') {
  ...
  getSuggestedActions(intent: any, language: string): string[] {
    const actions = {
      en: {
  ...
  getLeadScore(conversationId: string): number {
