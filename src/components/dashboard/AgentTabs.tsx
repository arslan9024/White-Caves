  ...
  return (
    <div style={S.tabContainer}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: `repeat(${stages.length}, minmax(200px, 1fr))`, gap: '0.75rem', overflowX: 'auto' }}>
        {grouped.map((g) => (
          <div key={g.stage} style={{ background: '#f9fafb', borderRadius: '12px', padding: '0.75rem', minHeight: '200px' }}>
            ...
            {g.items.map((l) => (
              <div key={l.id} style={{ ...S.card, marginBottom: '0.5rem', padding: '0.75rem' }}>
                ...
                <p style={{ fontSize: '0.8rem', color: '#6b7280', margin: '0.25rem 0 0 0' }}>
                  {l.tenant?.name ?? 'Pending'} — {S.formatCurrency(l.monthlyRent ?? 0)}
                </p>
              </div>
            ))}
          </div>
            ...
            {properties.map((p) => (
              <div key={p.id} style={S.card}>
                ...
                <div style={{ display: 'flex', gap: '0.75rem', marginTop: '0.5rem', fontSize: '0.85rem', color: '#6b7280' }}>
                  ...
                  <span style={{ fontWeight: 600, color: 'var(--color-primary, #D4AF37)' }}>{S.formatCurrency(p.price ?? 0)}/yr</span>
                  <span style={S.badge(
              ...
              <tbody>
                {leases.map((l) => (
                  <tr key={l.id}>
                    ...
                    <td style={{ ...S.td, fontWeight: 600 }}>{S.formatCurrency(l.monthlyRent ?? 0)}</td>
                    <td style={S.td}>
                      <span style={S.badge(
  ...
  return (
    <div style={S.tabContainer}>
      <div style={S.pageHeader}>
        ...
        <p style={S.headerSubtitle}>{upcoming.length} upcoming, {past.length} completed</p>
      </div>
      ...
      {viewings.length === 0
        ? S.emptyState('👁️', 'No viewings scheduled', 'Schedule viewings to show properties to prospective tenants.')
        : (
          <div style={S.tableWrapper}>
            <table style={S.table}>
              <thead>
  ...
  return (
    <div style={S.tabContainer}>
      ...
      {applications.length === 0
        ? S.emptyState('📋', 'No applications', 'Tenant applications will appear when prospective tenants apply.')
        : (
          <div style={S.tableWrapper}>
            <table style={S.table}>
              <thead>
...
export const LeaseRenewals: React.FC = () => {
  ...
  return (
    <div style={S.tabContainer}>
      ...
      {expiring.length === 0
        ? S.emptyState('🔄', 'No upcoming renewals', 'All leases are current. Renewals will appear as end dates approach.')
        : (
          <div style={S.tableWrapper}>
            <table style={S.table}>
              <thead>
              ...
              <tbody>
                {expiring.map((l) => (
                  <tr key={l.id}>
                    ...
                    <td style={S.td}>{S.formatCurrency(l.monthlyRent ?? 0)}</td>
                    <td style={S.td}>
              ...
              <tbody>
                {leads.map((l) => (
                  <tr key={l.id}>
                    ...
                    <td style={S.td}>{S.formatCurrency(l.budget ?? 0)}</td>
                    <td style={S.td}>
                      <span style={S.badge(
  ...
  return (
    <div style={S.tabContainer}>
      ...
      <div style={S.card}>
        ...
        <div style={{ background: '#f3f4f6', borderRadius: '8px', overflow: 'hidden', height: '24px', marginTop: '0.5rem' }}>
          <div
            style={{
              height: '100%',
