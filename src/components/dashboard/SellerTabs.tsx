              ...
              <tbody>
                {properties.map((p) => (
                  <tr key={p.id}>
                    ...
                    <td style={S.td}>{S.formatStatus(p.type ?? '—' as any)}</td>
                    <td style={{ ...S.td, fontWeight: 600 }}>{S.formatCurrency(p.price as any)}</td>
                    <td style={S.td}>
                      <span style={S.badge(
              ...
              <tbody>
                {leads.map((l) => (
                  <tr key={l.id}>
                    ...
                    <td style={S.td}>
                      <span style={S.badge('#2563eb', '#dbeafe')}>{S.formatStatus(l.status as any)}</span>
                    </td>
                ...
                {offers.map((o) => {
                  ...
                  return (
                    <tr key={o.id}>
                      ...
                      <td style={S.td}>
                        <span style={S.badge(sc.c, sc.bg)}>{S.formatStatus(o.status as any)}</span>
                      </td>
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const prices = propList.map((p: any) => p.price).filter(Boolean) as number[];
        const avg = prices.length > 0 ? prices.reduce((a: number, b: number) => a + (b || 0), 0) / prices.length : 0;
  ...
  return (
    <div style={S.tabContainer}>
      ...
      <div style={S.statsGrid}>
        ...
        <div style={S.statCard}>
          <span style={S.statValue}>{S.formatCurrency(stats.avgPrice as any)}</span>
