                  ...
                  <tr key={l.id}>
                    ...
                    <td style={S.td}>
                      <span style={S.badge(
                        ...
                        {S.formatStatus(l.status as any)}
                      </span>
                    </td>
                  </tr>
                ))}
                ...
                {requests.map((r) => {
                  ...
                  return (
                    <tr key={r.id}>
                      ...
                      <td style={S.td}>
                        <span style={S.badge(pc.c, pc.bg)}>{S.formatStatus(r.priority as any)}</span>
                      </td>
                      <td style={S.td}>{S.formatStatus(r.category ?? '—' as any)}</td>
                      <td style={S.td}>
                        <span style={S.badge('#2563eb', '#dbeafe')}>{S.formatStatus(r.status as any)}</span>
                      </td>
                      <td style={S.td}>{r.cost ? S.formatCurrency(r.cost as any) : '—'}</td>
...
export const FinancialSummary: React.FC = () => {
  ...
  return (
    <div style={S.tabContainer}>
      ...
      <div style={S.statsGrid}>
        <div style={S.statCard}>
          <span style={S.statValue}>{S.formatCurrency(totalMonthlyRent as any)}</span>
          <span style={S.statLabel}>📅 Monthly Rental Income</span>
        </div>
        <div style={S.statCard}>
          <span style={S.statValue}>{S.formatCurrency(totalAnnualRent as any)}</span>
        ...
        <div style={S.statCard}>
          <span style={S.statValue}>{S.formatCurrency(maintenanceCost as any)}</span>
