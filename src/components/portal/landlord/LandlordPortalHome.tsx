...
const LandlordPortalHome: FC<LandlordPortalHomeProps> = ({ onNavigate }) => {
  ...
  useEffect(() => {
    ...
    const fetchMaintenanceItems = async () => {
      ...
      const totalPages = firstPage.pagination?.totalPages ?? Math.max(1, Math.ceil(total / 50));
  ...
  const metrics = useMemo(() => {
    ...
    const occupancyRisk = leases.filter(
      l =>
        l.status === 'expiring' || (!!l.nextPaymentDue && new Date(l.nextPaymentDue) < new Date())
    ).length;
