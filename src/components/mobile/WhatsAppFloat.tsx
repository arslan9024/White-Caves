...
const WhatsAppFloat: React.FC = () => {
  ...
  // Routing logic: If on CRM/Dashboard, route to Henry/Internal Support. Otherwise route to Nadia/Sales.
  const isInternal =
    location.pathname.startsWith('/crm') || location.pathname.startsWith('/dashboard');

  const phoneNumber = isInternal
    ? (Config.COMPANY as any).INTERNAL_SUPPORT_WHATSAPP || '+971500000000'
    : Config.COMPANY.WHATSAPP || '+971505110636';
  const message = encodeURIComponent(
    isInternal
      ? 'Hello, I need internal CRM support.'
      : 'Hello! I would like to inquire about White Caves properties.'
  );
