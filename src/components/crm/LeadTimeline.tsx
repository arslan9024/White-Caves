  ...
  const submitNote = async () => {
    ...
    try {
      ...
      if (mounted.current) {
        ...
        setNoteOpen(true);
        setNoteText(optimistic.description);
      }
    }
  };
      ...
      {loading && (
        <div className="space-y-3">
          {[1, 2, 3].map(i => (
            <div
              key={i}
              className="backdrop-blur-sm bg-white/5 border border-white/10 rounded-2xl p-4 animate-pulse"
            >
          ...
          <AnimatePresence initial={false}>
            {visible.map(act => {
              ...
              return (
                ...
                >
                  <span
                    className={`w-9 h-9 rounded-xl flex items-center justify-center text-lg flex-shrink-0 ${meta.bg}`}
                    aria-hidden="true"
                  >
                    {meta.icon}
                  </span>
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-white/30 text-xs mt-1">
                      {new Date(act.createdAt).toLocaleString('en-AE', {
                        month: 'short',
                        day: 'numeric',
                        hour: '2-digit',
                        minute: '2-digit',
