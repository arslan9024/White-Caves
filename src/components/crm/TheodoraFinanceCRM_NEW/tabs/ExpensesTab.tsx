    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', fontSize: '0.85rem' }}>
          ...
          <tbody>
            {filteredCatalogItems.map(item => (
              <tr key={item.expense_id} style={{ borderBottom: '1px solid #F1F5F9' }}>
                <td style={{ padding: '12px 16px' }}>
                  <div style={{ fontWeight: 800, color: '#1E293B' }}>{item.expense_id}</div>
                  <div style={{ fontSize: '0.72rem', color: '#8B5CF6', fontWeight: 800 }}>Ledger: {item.accounting_ledger_code}</div>
                </td>
                <td style={{ padding: '12px 16px' }}>
                  <div style={{ fontWeight: 700, color: '#1E293B' }}>{item.name}</div>
                  <div style={{ fontSize: '0.75rem', color: '#64748B', maxWidth: '380px' }}>{item.description}</div>
                </td>
                <td style={{ padding: '12px 16px' }}>
                  <span style={{ display: 'inline-block', padding: '2px 8px', borderRadius: '4px', fontSize: '0.72rem', fontWeight: 700, background: '#F1F5F9', color: '#334155' }}>
                    {item.category_id}
                  </span>
                </td>
                <td style={{ padding: '12px 16px' }}>
                  <span style={{ fontWeight: 800, color: item.uae_vat_rate > 0 ? '#10B981' : '#64748B' }}>
                    {item.uae_vat_rate > 0 ? '5.0% (Input VAT)' : '0.0% (Exempt)'}
                  </span>
                </td>
                <td style={{ padding: '12px 16px' }}>
                  <span style={{ padding: '2px 8px', borderRadius: '4px', fontSize: '0.72rem', fontWeight: 800, background: item.fta_ct_deductible ? '#ECFDF5' : '#FEF2F2', color: item.fta_ct_deductible ? '#047857' : '#B91C1C' }}>
      ...
      {showAddExpenseModal && (
        <div style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 9999 }}>
          <div style={{ background: '#FFFFFF', padding: '2rem', borderRadius: '16px', width: '520px', maxWidth: '90%', maxHeight: '90vh', overflowY: 'auto' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              ...
              <div style={{ background: '#F8FAFC', padding: '1rem', borderRadius: '8px', border: '1px solid #E2E8F0' }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.9rem', fontWeight: 800, color: '#1E293B', marginTop: '4px' }}>
              ...
              <div style={{ display: 'flex', gap: '0.75rem', marginTop: '0.5rem' }}>
                <button
                  ...
                  style={{ flex: 1, padding: '10px', borderRadius: '8px', border: '1px solid #CBD5E1', background: '#FFFFFF', color: '#475569', fontWeight: 700, cursor: 'pointer' }}
                >
                  Cancel
                </button>
                <button
                  ...
                  style={{ flex: 1, padding: '10px', borderRadius: '8px', border: 'none', background: '#8B5CF6', color: '#FFFFFF', fontWeight: 800, cursor: 'pointer' }}
                >
