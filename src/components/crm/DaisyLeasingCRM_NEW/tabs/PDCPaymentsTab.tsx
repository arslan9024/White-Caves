...
const STATUS_FILTERS: Array<PDCStatus | 'all'> = [
  'all',
  'pending',
  'presented',
  'cleared',
...
const PDCPaymentsTab: React.FC<PDCPaymentsTabProps> = ({ pdcCheques, onUpdateStatus }) => {
  ...
  const filtered =
    filterStatus === 'all' ? localCheques : localCheques.filter(c => c.status === filterStatus);
  ...
  const clearedValue = localCheques
      ...
      <div className="view-header" style={{ marginBottom: '16px' }}>
        <div>
          ...
          <p style={{ margin: '4px 0 0', fontSize: '12px', color: 'var(--color-text-secondary)' }}>
            {stats.total} cheques · AED {totalValue.toLocaleString()} total · AED{' '}
            {clearedValue.toLocaleString()} cleared
          </p>
        </div>
      </div>
            ...
            return (
              ...
              >
                ...
                >
                  {cheque.dueDate}
                  {overdue && (
                    <span style={{ marginLeft: '4px', fontSize: '10px' }}>🔴 Overdue</span>
                  )}
                  {dueSoon && !overdue && (
                    <span style={{ marginLeft: '4px', fontSize: '10px' }}>⚠️ Soon</span>
                  )}
                </span>
                <span>
                ...
                <span style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
                  {cheque.status === 'pending' && (
                    <button
                      onClick={() => handleUpdate(cheque.id, 'presented')}
                      style={{
                        padding: '4px 8px',
                        borderRadius: '4px',
                  ...
                  {cheque.status === 'presented' && (
                    <>
                      <button
                        onClick={() => handleUpdate(cheque.id, 'cleared')}
                        style={{
                          padding: '4px 8px',
                          borderRadius: '4px',
                      ...
                      <button
                        onClick={() => handleUpdate(cheque.id, 'bounced')}
                        style={{
                          padding: '4px 8px',
                          borderRadius: '4px',
