  ...
  return (
    <div className="maintenance-view">
      <div className="view-header">
        ...
        <button className="add-btn" style={{ background: 'linear-gradient(135deg, #14B8A6 0%, #10B981 100%)' }}>
          + New Request
        </button>
      </div>
      ...
      <div style={{ display: 'flex', gap: '16px', flexWrap: 'wrap', marginBottom: '16px' }}>
        <div>
          <span style={{ fontSize: '12px', color: 'var(--color-text-secondary)', marginRight: '8px' }}>Priority:</span>
          <div className="filter-buttons" style={{ display: 'inline-flex', gap: '6px' }}>
            {(['all', ...PRIORITY_ORDER] as FilterPriority[]).map(p => (
              <button
                ...
                style={{
                  padding: '5px 12px', borderRadius: '6px', border: '1px solid', fontSize: '12px', cursor: 'pointer',
                  borderColor: filterPriority === p ? '#14B8A6' : 'var(--color-border-default)',
                  background: filterPriority === p ? 'rgba(20,184,166,0.15)' : 'var(--rgba-white-05)',
        ...
        <div>
          <span style={{ fontSize: '12px', color: 'var(--color-text-secondary)', marginRight: '8px' }}>Status:</span>
          <div className="filter-buttons" style={{ display: 'inline-flex', gap: '6px' }}>
            {(['all', 'pending', 'in_progress', 'scheduled', 'completed', 'cancelled'] as FilterStatus[]).map(s => (
              <button
                key={s}
      ...
      <div className="maintenance-list">
        {filtered.length === 0 && (
          <div style={{ padding: '40px', textAlign: 'center', color: 'var(--color-text-secondary)', fontSize: '14px', background: 'var(--rgba-white-05)', borderRadius: '12px' }}>
        ...
        {filtered.map((req: MaintenanceRequest) => {
          ...
          return (
            <div key={req.id} className={`maintenance-card ${req.priority}`}
              style={{ flexDirection: 'column', alignItems: 'stretch', borderLeft: `3px solid ${pCol.text}` }}
            >
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '8px' }}>
                <div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '2px' }}>
                    ...
                    <span style={{ fontSize: '11px', color: 'var(--color-text-secondary)' }}>— {req.building}</span>
                  </div>
                  <span style={{ fontSize: '12px', color: 'var(--color-text-secondary)' }}>{req.tenant}</span>
                </div>
                <div style={{ display: 'flex', gap: '6px', alignItems: 'center' }}>
                  <span style={{ fontSize: '11px', padding: '2px 8px', borderRadius: '4px', background: `${catCol}22`, color: catCol, border: `1px solid ${catCol}44` }}>
                    {req.category}
                  </span>
                  <span className={`priority-badge ${req.priority}`} style={{ fontSize: '11px', padding: '2px 8px', borderRadius: '4px', background: pCol.bg, color: pCol.text }}>
                    {req.priority}
                  </span>
                  <span style={{ fontSize: '11px', padding: '2px 8px', borderRadius: '4px', background: sCol.bg, color: sCol.text }}>
                    {req.status.replace('_', ' ')}
                  </span>
                </div>
              </div>

              <p className="issue" style={{ margin: '0 0 10px', fontSize: '13px', color: 'var(--color-text-primary)', lineHeight: 1.5 }}>
                {req.issue}
              </p>

              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: '8px', fontSize: '12px', color: 'var(--color-text-secondary)', marginBottom: '10px' }}>
                ...
                <div style={{ display: 'flex', gap: '6px' }}>
                  {req.status === 'pending' && (
                    <>
                      <button onClick={() => onUpdateStatus?.(req.id, 'in_progress')}
                        style={{ padding: '5px 10px', borderRadius: '6px', border: '1px solid #3B82F6', background: 'rgba(59,130,246,0.1)', color: '#60A5FA', fontSize: '12px', cursor: 'pointer' }}>
                        Mark In Progress
                      </button>
                      <button style={{ padding: '5px 10px', borderRadius: '6px', border: '1px solid #14B8A6', background: 'rgba(20,184,166,0.1)', color: '#14B8A6', fontSize: '12px', cursor: 'pointer' }}>
                        Assign
                      </button>
                    </>
                  )}
                  {req.status === 'in_progress' && (
                    <button onClick={() => onUpdateStatus?.(req.id, 'completed')}
                      style={{ padding: '5px 10px', borderRadius: '6px', border: '1px solid #10B981', background: 'rgba(16,185,129,0.1)', color: '#10B981', fontSize: '12px', cursor: 'pointer' }}>
                      Mark Complete
                    </button>
                  )}
                  ...
                  {(req.status !== 'completed' && req.status !== 'cancelled') && (
                    <button onClick={() => onUpdateStatus?.(req.id, 'cancelled')}
                      style={{ padding: '5px 10px', borderRadius: '6px', border: '1px solid #EF4444', background: 'rgba(239,68,68,0.1)', color: '#EF4444', fontSize: '12px', cursor: 'pointer' }}>
                      Cancel
                    </button>
                  )}
                </div>
