...
const STATUS_FILTERS: Array<InquiryStatus | 'all'> = [
  'all', 'new', 'viewing_scheduled', 'offer_made', 'documents_pending', 'approved', 'rejected',
];
  ...
  return (
    <div className="inquiries-view">
      <div className="view-header">
        ...
        <button className="add-btn"><Plus size={16} /> Add Inquiry</button>
      </div>
      ...
      <div className="inquiry-cards">
        {filtered.map((inq: RentalInquiry) => {
          const stageLabel = LEASING_STAGE_LABELS[inq.leasingStage as LeasingStage] ?? `Stage ${inq.leasingStage}`;
            ...
            <div key={inq.id} className="inquiry-card">
              ...
              <div style={{ marginBottom: '10px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '4px' }}>
                  ...
                  <span style={{ fontSize: '11px', color: '#14B8A6', fontWeight: 600 }}>{stagePct}%</span>
                </div>
                <div style={{ height: '6px', background: 'var(--rgba-white-10)', borderRadius: '3px', overflow: 'hidden' }}>
                  <div style={{ width: `${stagePct}%`, height: '100%', background: 'linear-gradient(90deg, #14B8A6 0%, #10B981 100%)', borderRadius: '3px', transition: 'width 0.3s' }} />
                </div>
              </div>

              {inq.notes && (
                <p style={{ margin: '0 0 10px', fontSize: '12px', color: 'var(--color-text-secondary)', lineHeight: 1.4, borderLeft: '2px solid var(--rgba-white-10)', paddingLeft: '8px' }}>
                  {inq.notes.length > 90 ? `${inq.notes.slice(0, 90)}…` : inq.notes}
                </p>
              )}

              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
                <span style={{ fontSize: '11px', padding: '2px 8px', borderRadius: '4px', background: `${sourceColor}22`, color: sourceColor, border: `1px solid ${sourceColor}44` }}>
                  {inq.source}
                </span>
                <span style={{ fontSize: '11px', color: 'var(--color-text-secondary)' }}>{inq.date}</span>
              </div>

              <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                <button style={{ display: 'flex', alignItems: 'center', gap: '4px', padding: '6px 12px', borderRadius: '6px', border: '1px solid var(--color-border-default)', background: 'var(--rgba-white-05)', color: 'var(--color-text-secondary)', fontSize: '12px', cursor: 'pointer' }}>
                  <Phone size={12} /> Call
                </button>
                <button style={{ display: 'flex', alignItems: 'center', gap: '4px', padding: '6px 12px', borderRadius: '6px', border: '1px solid var(--color-border-default)', background: 'var(--rgba-white-05)', color: 'var(--color-text-secondary)', fontSize: '12px', cursor: 'pointer' }}>
                  <Mail size={12} /> Email
                ...
                <button style={{ display: 'flex', alignItems: 'center', gap: '4px', padding: '6px 12px', borderRadius: '6px', border: '1px solid #14B8A6', background: 'rgba(20,184,166,0.1)', color: '#14B8A6', fontSize: '12px', cursor: 'pointer', marginLeft: 'auto' }}>
                  Advance ➡
                </button>
              </div>
            </div>
        ...
        {filtered.length === 0 && (
          <div style={{ gridColumn: '1 / -1', padding: '40px', textAlign: 'center', color: 'var(--color-text-secondary)', fontSize: '14px' }}>
