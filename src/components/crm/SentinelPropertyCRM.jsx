...
export const SentinelPropertyCRM: React.FC<SentinelProps> = ({ moduleId }) => {
  ...
  return (
    <div className="crm-container" style={{ maxWidth: '100%', padding: '0.5rem' }}>
      {/* Header */}
      <div style={{ background: 'linear-gradient(135deg, var(--color-1e3a8a, #1E3A8A) 0%, var(--color-172554, #172554) 100%)', color: 'var(--white, #FFFFFF)', padding: '1.25rem 1.5rem', borderRadius: '16px', marginBottom: '1.25rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '1rem' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
          <div style={{ width: '48px', height: '48px', borderRadius: '12px', background: 'linear-gradient(135deg, var(--accent-blue, #3B82F6) 0%, var(--accent-blue, #2563EB) 100%)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.5rem' }}>
            🏢
          </div>
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
              <h2 style={{ margin: 0, fontSize: '1.3rem', fontWeight: 800 }}>Sentinel AI — Property State Machine & Quality</h2>
              <span style={{ fontSize: '0.7rem', background: 'rgba(255, 255, 255, 0.15)', padding: '2px 8px', borderRadius: '4px', color: 'var(--color-bfdbfe, #BFDBFE)', fontWeight: 800 }}>
                Inventory Quality Control
              </span>
            </div>
            <p style={{ margin: '0.2rem 0 0 0', fontSize: '0.82rem', color: 'var(--color-dbeafe, #DBEAFE)' }}>
      ...
      <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap', background: 'var(--white, #FFFFFF)', padding: '8px 12px', borderRadius: '12px', border: '1px solid var(--text-secondary, #E2E8F0)', marginBottom: '1.25rem' }}>
        <button onClick={() => setActiveTab('lifecycle')} style={{ padding: '6px 12px', borderRadius: '6px', border: activeTab === 'lifecycle' ? '1px solid var(--accent-blue, #2563EB)' : '1px solid transparent', background: activeTab === 'lifecycle' ? 'var(--accent-blue, #2563EB)' : 'var(--color-f8fafc, #F8FAFC)', color: activeTab === 'lifecycle' ? 'var(--white, #FFF)' : 'var(--color-334155, #334155)', fontWeight: 800, fontSize: '0.8rem', cursor: 'pointer' }}>
          3.20.1 Lifecycle State Machine
        </button>
        <button onClick={() => setActiveTab('verification')} style={{ padding: '6px 12px', borderRadius: '6px', border: activeTab === 'verification' ? '1px solid var(--accent-blue, #2563EB)' : '1px solid transparent', background: activeTab === 'verification' ? 'var(--accent-blue, #2563EB)' : 'var(--color-f8fafc, #F8FAFC)', color: activeTab === 'verification' ? 'var(--white, #FFF)' : 'var(--color-334155, #334155)', fontWeight: 800, fontSize: '0.8rem', cursor: 'pointer' }}>
          3.20.2 Listing Quality Gate
        </button>
        <button onClick={() => setActiveTab('decommission')} style={{ padding: '6px 12px', borderRadius: '6px', border: activeTab === 'decommission' ? '1px solid var(--accent-blue, #2563EB)' : '1px solid transparent', background: activeTab === 'decommission' ? 'var(--accent-blue, #2563EB)' : 'var(--color-f8fafc, #F8FAFC)', color: activeTab === 'decommission' ? 'var(--white, #FFF)' : 'var(--color-334155, #334155)', fontWeight: 800, fontSize: '0.8rem', cursor: 'pointer' }}>
          3.20.3 Decommission Flow
        </button>
        <button onClick={() => setActiveTab('telemetry')} style={{ padding: '6px 12px', borderRadius: '6px', border: activeTab === 'telemetry' ? '1px solid var(--accent-blue, #2563EB)' : '1px solid transparent', background: activeTab === 'telemetry' ? 'var(--accent-blue, #2563EB)' : 'var(--color-f8fafc, #F8FAFC)', color: activeTab === 'telemetry' ? 'var(--white, #FFF)' : 'var(--color-334155, #334155)', fontWeight: 800, fontSize: '0.8rem', cursor: 'pointer' }}>
      ...
      {activeTab === 'lifecycle' && (
        <div style={{ background: 'var(--white, #FFFFFF)', borderRadius: '12px', border: '1px solid var(--text-secondary, #E2E8F0)', overflow: 'hidden' }}>
          <div style={{ padding: '0.75rem 1rem', background: 'var(--color-f8fafc, #F8FAFC)', borderBottom: '1px solid var(--text-secondary, #E2E8F0)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <span style={{ fontWeight: 800, fontSize: '0.9rem', color: 'var(--color-1e293b, #1E293B)' }}>Managed Units State Machine</span>
            <span style={{ fontSize: '0.75rem', color: 'var(--accent-blue, #2563EB)', fontWeight: 800 }}>Click Stage to Transition</span>
          </div>
            ...
            <tbody>
              {properties.map(p => (
                <tr key={p.id} style={{ borderBottom: '1px solid var(--color-f1f5f9, #F1F5F9)' }}>
                  <td style={{ padding: '10px 14px', fontWeight: 800, color: 'var(--accent-blue, #2563EB)' }}>{p.id}</td>
                  <td style={{ padding: '10px 14px', fontWeight: 700, color: 'var(--color-1e293b, #1E293B)' }}>{p.title}</td>
                  <td style={{ padding: '10px 14px', color: 'var(--text-secondary, #64748B)' }}>AED {p.priceAed}</td>
                  <td style={{ padding: '10px 14px' }}>
                    <span style={{ padding: '3px 8px', borderRadius: '4px', fontSize: '0.75rem', fontWeight: 800, background: p.currentStage === 'LISTED' ? 'var(--color-ecfdf5, #ECFDF5)' : p.currentStage === 'UNDER_OFFER' ? 'var(--color-fef3c7, #FEF3C7)' : 'var(--color-eff6ff, #EFF6FF)', color: p.currentStage === 'LISTED' ? 'var(--color-047857, #047857)' : p.currentStage === 'UNDER_OFFER' ? 'var(--color-b45309, #B45309)' : 'var(--color-1e40af, #1E40AF)' }}>
                      {p.currentStage}
                    </span>
                  </td>
                  <td style={{ padding: '10px 14px' }}>
                    <button onClick={() => advanceStage(p.id)} style={{ background: 'var(--color-f1f5f9, #F1F5F9)', border: '1px solid var(--text-secondary, #CBD5E1)', borderRadius: '6px', padding: '4px 10px', fontSize: '0.75rem', fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '4px' }}>
        ...
        <div style={{ background: 'var(--white, #FFFFFF)', padding: '1.5rem', borderRadius: '12px', border: '1px solid var(--text-secondary, #E2E8F0)' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', marginBottom: '1.25rem' }}>
            {Object.entries(checklist).map(([key, val]) => (
              <label key={key} style={{ display: 'flex', alignItems: 'center', gap: '10px', fontSize: '0.85rem', fontWeight: 700, color: 'var(--color-1e293b, #1E293B)', cursor: 'pointer', background: 'var(--color-f8fafc, #F8FAFC)', padding: '10px 14px', borderRadius: '8px', border: '1px solid var(--text-secondary, #E2E8F0)' }}>
          ...
          <div style={{ background: isGateApproved ? 'var(--color-ecfdf5, #ECFDF5)' : 'var(--color-fef2f2, #FEF2F2)', border: `1px solid ${isGateApproved ? 'var(--color-a7f3d0, #A7F3D0)' : 'var(--color-fecaca, #FECACA)'}`, padding: '1rem', borderRadius: '8px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div>
              <div style={{ fontWeight: 800, color: isGateApproved ? 'var(--color-065f46, #065F46)' : 'var(--color-991b1b, #991B1B)', fontSize: '0.9rem' }}>
            ...
            {isGateApproved && (
              <button style={{ background: 'var(--color-047857, #047857)', color: 'var(--white, #FFF)', border: 'none', borderRadius: '6px', padding: '6px 14px', fontWeight: 800, fontSize: '0.78rem', cursor: 'pointer' }}>
                Publish Syndication
              </button>
            )}
          </div>
      ...
      {activeTab === 'telemetry' && (
        <div style={{ background: 'var(--white, #FFFFFF)', padding: '1.5rem', borderRadius: '12px', border: '1px solid var(--text-secondary, #E2E8F0)' }}>
          ...
          <div style={{ background: 'var(--color-eff6ff, #EFF6FF)', border: '1px solid var(--color-bfdbfe, #BFDBFE)', padding: '1rem', borderRadius: '8px', display: 'flex', alignItems: 'center', gap: '8px', color: 'var(--accent-blue, #1D4ED8)', fontWeight: 800 }}>
            <Activity size={18} />
