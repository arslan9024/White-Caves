    ...
    <div className="qr-code-container">
      ...
      <div className="qr-details">
        <p className="qr-amount">
          Amount: <strong>{displayAmount}</strong>
        </p>
  ...
  <div className="cheque-instructions">
    <div className="instruction-card">
      ...
      <ol className="instruction-list">
        <li>
          Make the cheque payable to: <strong>{payeeName || BANK_DETAILS.accountName}</strong>
        </li>
        <li>
          Amount:{' '}
          <strong>{amount != null ? `AED ${amount.toLocaleString()}` : 'Amount pending'}</strong>{' '}
          (in words on the cheque)
        </li>
        <li>
          Write reference number on the back: <strong>{reference}</strong>
        </li>
...
const BankTransferDetails = memo(
  ({ amount, reference, onCopy, copied }: BankTransferDetailsProps) => (
    <div className="bank-transfer-details">
      ...
      <div className="details-grid">
        <div className="detail-row">
          ...
          <button
            className="copy-icon"
            onClick={() => onCopy(BANK_DETAILS.accountName, 'accountName')}
        ...
        <div className="detail-row">
          ...
          <button
            className="copy-icon"
            onClick={() => onCopy(BANK_DETAILS.accountNumber, 'accountNumber')}
        ...
        <div className="detail-row">
          ...
          <button
            className="copy-icon"
            onClick={() => onCopy(BANK_DETAILS.iban, 'iban')}
        ...
        <div className="detail-row">
          ...
          <button
            className="copy-icon"
            onClick={() => onCopy(BANK_DETAILS.routingCode, 'routing')}
            aria-label="Copy routing code"
          >
        ...
        <div className="detail-row">
          ...
          <button
            className="copy-icon"
            onClick={() => onCopy(BANK_DETAILS.swiftCode, 'swift')}
        ...
        <div className="detail-row highlight">
          ...
          <span className="detail-value">
            {amount != null ? `AED ${amount.toLocaleString()}` : 'Amount pending'}
          </span>
        </div>
        <div className="detail-row highlight">
...
const PaymentInstructionDeck = memo(
  ({
    ...
    const createPaymentIntent = useCallback(async () => {
      ...
      if (!amount || amount <= 0) {
        setIntentError('Please provide a valid amount before generating a payment link.');
        return;
      }
      ...
      try {
        ...
        if (!response.ok || !payload.success) {
          setIntentError(payload.error || 'Failed to generate payment link.');
          return;
        }

        setIntentStatus(`Payment link generated (${payload.data?.paymentIntentId ?? 'stub'}).`);
      } catch {
        setIntentError('Failed to generate payment link. Please try again.');
      } finally {
        setIntentLoading(false);
      }
    }, [amount, clientName, invoiceId, reference]);
    ...
    return (
      <div className="payment-instruction-deck">
        <div
          ...
          aria-label="Toggle payment instructions"
        >
        ...
        {expanded && (
          <div className="deck-content">
            ...
            <div className="payment-details">
              {selectedMethod === 'qr' && (
                <QRCodeDisplay amount={amount} reference={reference} onCopy={handleCopy} />
              )}
            ...
            <div className="deck-actions">
              ...
              >
                <CreditCard size={16} />
                {intentLoading ? 'Generating…' : 'Generate Payment Link'}
              </button>
            ...
            {intentStatus && <p className="payment-intent-success">{intentStatus}</p>}
            {intentError && <p className="payment-intent-error">{intentError}</p>}
          </div>
        )}
      </div>
    );
