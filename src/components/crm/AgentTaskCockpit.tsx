  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (Array.isArray(json)) {
        ...
        setSummary({
          total: json.length,
          breached: fallback.overdue.length,
          today: fallback.today.length,
        });
      ...
      <div className="bg-[#0A0A0A] min-h-screen p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {(['overdue', 'today', 'upcoming'] as Column[]).map(c => (
            <div key={c}>
              <div className="h-5 bg-white/10 rounded w-24 mb-4 animate-pulse" />
              {[1, 2, 3].map(i => (
                <Skeleton key={i} />
              ))}
            </div>
          ...
          return (
            <div key={col}>
              <div className={`flex items-center gap-2 mb-4 pb-2 border-b ${cfg.border}`}>
                ...
                <h2 className={`text-sm font-semibold uppercase tracking-wider ${cfg.accent}`}>
                  {cfg.label}
                </h2>
              ...
              <motion.div
                ...
                initial="hidden"
                animate="visible"
                aria-label={`${cfg.label} leads`}
              >
                {leads.length === 0 && (
                  <motion.p variants={card} className="text-white/30 text-sm text-center py-8">
                    No leads here
                  </motion.p>
                )}
                {leads.map(lead => (
                  <motion.div
                    key={lead.id}
                    variants={card}
                    className={`backdrop-blur-sm bg-white/5 border ${cfg.border} rounded-2xl p-4`}
                  >
                    <div className="flex items-start justify-between mb-1">
                      ...
                      <span
                        className={`text-xs px-2 py-0.5 rounded-full font-medium ${lead.slaRisk === 'breached' ? 'bg-red-500/20 text-red-300' : lead.status === 'new' ? 'bg-blue-500/20 text-blue-300' : 'bg-green-500/20 text-green-300'}`}
                      >
                        {lead.slaRisk === 'breached'
                          ? 'SLA Risk'
                          : lead.status === 'new'
                            ? 'New'
                    ...
                    {lead.propertyTitle && (
                      <p className="text-white/40 text-xs mb-1">{lead.propertyTitle}</p>
                    )}
