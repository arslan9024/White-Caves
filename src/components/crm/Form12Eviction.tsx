  ...
  return (
    <div style={{ padding: '24px', backgroundColor: '#FFFFFF', borderRadius: '12px', border: '1px solid #E2E8F0' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '20px' }}>
        <div>
          <h3 style={{ margin: 0, color: '#1E293B', fontSize: '18px', fontWeight: 'bold' }}>Form 12 Eviction Notice Timeline</h3>
          <p style={{ margin: '4px 0 0 0', color: '#64748B', fontSize: '13px' }}>
            Statutory 12-Month Notice Tracking under Dubai Law No. 33 of 2008 (Notary Public Served Notices)
          </p>
        </div>
        <div style={{ padding: '6px 12px', backgroundColor: 'rgba(239, 68, 68, 0.1)', color: '#EF4444', borderRadius: '20px', fontSize: '12px', fontWeight: 'bold' }}>
        ...
        <tbody>
          {records.map(r => (
            <tr key={r.id} style={{ borderBottom: '1px solid #F1F5F9' }}>
              <td style={{ padding: '12px', fontWeight: 'bold', color: '#EF4444', fontFamily: 'monospace' }}>{r.id}</td>
              <td style={{ padding: '12px' }}>
                <div style={{ fontWeight: '600', color: '#1E293B' }}>{r.propertyTitle}</div>
                <div style={{ fontSize: '11px', color: '#64748B' }}>{r.tenantName}</div>
              </td>
              <td style={{ padding: '12px', color: '#475569', fontFamily: 'monospace' }}>{r.notaryReference}</td>
              <td style={{ padding: '12px', color: '#475569' }}>{r.servedDate}</td>
              <td style={{ padding: '12px', fontWeight: 'bold', color: '#1E293B' }}>{r.expiryDate}</td>
              <td style={{ padding: '12px' }}>
                <span style={{ padding: '4px 8px', borderRadius: '4px', backgroundColor: r.daysRemaining < 60 ? 'rgba(239, 68, 68, 0.15)' : '#F1F5F9', color: r.daysRemaining < 60 ? '#EF4444' : '#1E293B', fontWeight: 'bold', fontSize: '11px' }}>
              ...
              <td style={{ padding: '12px' }}>
                <button
                  ...
                  style={{ padding: '6px 12px', backgroundColor: '#EF4444', color: '#FFFFFF', border: 'none', borderRadius: '6px', fontSize: '11px', fontWeight: 'bold', cursor: 'pointer' }}
                >
                  Send Notice
                </button>
              </td>
      ...
      {notificationLog.length > 0 && (
        <div style={{ backgroundColor: '#0F172A', color: '#38BDF8', padding: '16px', borderRadius: '8px', fontFamily: 'monospace', fontSize: '12px' }}>
          <div style={{ color: '#94A3B8', marginBottom: '8px', fontWeight: 'bold' }}>📡 STATUTORY NOTIFICATION DISPATCH LOG:</div>
