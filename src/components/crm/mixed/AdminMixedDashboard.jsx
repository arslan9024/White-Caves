...
import { Settings, Plug, BookOpen, Activity, Database, Server, Shield, Cpu } from 'lucide-react';
...
const ADMIN_STATS = [
  { id: 'docs', label: 'Documents', value: 156, icon: <BookOpen size={20} />, iconBg: 'rgba(59, 130, 246, 0.1)', highlight: true },
  { id: 'integrations', label: 'Integrations', value: 12, icon: <Plug size={20} />, iconBg: 'rgba(139, 92, 246, 0.1)' },
  { id: 'uptime', label: 'System Uptime', value: '99.9%', icon: <Activity size={20} />, iconBg: 'rgba(16, 185, 129, 0.1)' },
  { id: 'issues', label: 'Open Issues', value: 3, icon: <Shield size={20} />, iconBg: 'rgba(245, 158, 11, 0.1)' }
];
...
const RECENT_ACTIVITIES = [
  { id: 1, icon: 'success', actor: 'Aurora', action: 'updated documentation for', target: 'API v2.5', timestamp: '1 hour ago', type: 'success' },
  { id: 2, icon: 'ai', actor: 'Stella', action: 'optimized database queries', timestamp: '2 hours ago', type: 'default' },
  { id: 3, icon: 'success', actor: 'Nova', action: 'deployed hotfix for', target: 'Auth Module', timestamp: '4 hours ago', type: 'success' },
  { id: 4, icon: 'ai', actor: 'Marina', action: 'generated system report', timestamp: '6 hours ago', type: 'default' }
];
...
export default function AdminMixedDashboard({ subItem = 'settings', selectedAssistant }) {
  const [currentStage, setCurrentStage] = useState(2);
  const deptConfig = DEPT_ASSISTANT_MAP.admin;
  
  const handleStageClick = (stage, index) => setCurrentStage(index);
  const handleAction = (action) => console.log('Admin action:', action);
  ...
  return (
    <MixedDashboard
      ...
      assistantName={selectedAssistant ? selectedAssistant.charAt(0).toUpperCase() + selectedAssistant.slice(1) : 'Aurora'}
                ...
                <div key={i} style={{ 
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '10px' }}>
                    <div style={{ 
                      ...
                      background: system.status === 'operational' ? 'rgba(16, 185, 129, 0.1)' : 'rgba(245, 158, 11, 0.1)',
      ...
      activityComponent={<LiveActivityFeed activities={RECENT_ACTIVITIES} title="System Activity" />}
