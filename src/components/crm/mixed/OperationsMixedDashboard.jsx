...
import { Settings2, Users, Building2, Network, Calendar, UserPlus, Briefcase } from 'lucide-react';
...
const OPERATIONS_STATS = [
  { id: 'employees', label: 'Total Employees', value: 103, change: 5, icon: <Users size={20} />, iconBg: 'rgba(59, 130, 246, 0.1)', highlight: true },
  { id: 'departments', label: 'Departments', value: 10, icon: <Building2 size={20} />, iconBg: 'rgba(139, 92, 246, 0.1)' },
  { id: 'retention', label: 'Retention Rate', value: '94%', change: 2, icon: <Network size={20} />, iconBg: 'rgba(16, 185, 129, 0.1)' },
  { id: 'onboarding', label: 'Onboarding', value: 5, icon: <UserPlus size={20} />, iconBg: 'rgba(212, 175, 55, 0.1)' }
];

const OPERATIONS_LIFECYCLE = ['Recruitment', 'Onboarding', 'Training', 'Active', 'Review', 'Offboarding'];
...
const RECENT_ACTIVITIES = [
  { id: 1, icon: 'success', actor: 'Marcus', action: 'completed onboarding for', target: 'Ahmed (Sales)', timestamp: '1 hour ago', type: 'success' },
  { id: 2, icon: 'ai', actor: 'Kevin', action: 'scheduled team meeting for', target: 'Marketing Dept', timestamp: '2 hours ago', type: 'default' },
  { id: 3, icon: 'user', actor: 'HR', action: 'approved leave request for', target: 'Sarah Thompson', timestamp: '3 hours ago', type: 'success' },
  { id: 4, icon: 'pending', actor: 'System', action: 'performance review due for', target: '15 employees', timestamp: 'Today', type: 'pending' }
];

export default function OperationsMixedDashboard({ subItem = 'departments', selectedAssistant }) {
  const [currentStage, setCurrentStage] = useState(3);
  const deptConfig = DEPT_ASSISTANT_MAP.operations;
  
  const handleStageClick = (stage, index) => setCurrentStage(index);
  const handleAction = (action) => console.log('Operations action:', action);
  ...
  return (
    <MixedDashboard
      ...
      assistantName={selectedAssistant ? selectedAssistant.charAt(0).toUpperCase() + selectedAssistant.slice(1) : 'Marcus'}
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '12px' }}>
            {[
              ...
              <div key={i} style={{ 
                ...
                <span style={{ fontSize: '1.25rem', fontWeight: 700, color: '#D4AF37' }}>{dept.members}</span>
      ...
      activityComponent={<LiveActivityFeed activities={RECENT_ACTIVITIES} title="Operations Activity" />}
