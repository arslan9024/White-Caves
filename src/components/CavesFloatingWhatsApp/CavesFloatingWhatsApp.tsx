/**
 * CavesFloatingWhatsApp.tsx — View Layer (4-Way Component Architecture)
 * Sits at folder root: Pure presentational shell drawing data variables and logic hooks.
 */

import React, { FC } from 'react';
import { useCavesFloatingWhatsAppLogic, UseCavesFloatingWhatsAppProps } from './logic/CavesFloatingWhatsApp.logic';
import { WHATSAPP_WIDGET_TEXT } from './data/CavesFloatingWhatsApp.data';
import { FloatingBtn, SvgIcon } from './styles/CavesFloatingWhatsApp.style';

export interface CavesFloatingWhatsAppProps extends UseCavesFloatingWhatsAppProps {
  className?: string;
}

export const CavesFloatingWhatsApp: FC<CavesFloatingWhatsAppProps> = ({
  phoneNumber,
  customMessage,
  className,
}) => {
  const { handleClick } = useCavesFloatingWhatsAppLogic({ phoneNumber, customMessage });

  return (
    <FloatingBtn
      onClick={handleClick}
      aria-label={WHATSAPP_WIDGET_TEXT.ariaLabel}
      title={WHATSAPP_WIDGET_TEXT.tooltip}
      className={className}
      data-testid="caves-floating-whatsapp"
    >
...
export default CavesFloatingWhatsApp;
