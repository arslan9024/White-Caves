  ...
  useEffect(() => {
    const handleKeyPress = (e: KeyboardEvent) => {
      if ((e.ctrlKey || e.metaKey) && e.key === 'k') {
        e.preventDefault();
        (document.querySelector('.main-nav-search-input') as HTMLInputElement)?.focus();
      }
    };
  ...
  return (
    <header className="main-nav-bar">
      <div className="main-nav-left">
        {/* Left Sidebar Toggle */}
        <button
          className={`sidebar-toggle left-toggle ${isLeftSidebarOpen ? 'active' : ''}`}
      ...
      <div className="main-nav-right">
        ...
        <button
          className={`sidebar-toggle right-toggle ${isRightSidebarOpen ? 'active' : ''}`}
          onClick={onRightSidebarToggle}
          title={isRightSidebarOpen ? 'Hide AI Panel' : 'Show AI Panel'}
        >
          <PanelRight size={20} />
