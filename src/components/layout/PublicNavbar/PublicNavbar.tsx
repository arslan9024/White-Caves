...
const DESKTOP_NAV: DropdownGroup[] = [
  { label: 'Home', path: '/' },
  {
    label: 'Properties',
    path: '/properties',
    children: [
      ...PUBLIC_NAV.buy.slice(0, 3),
      ...PUBLIC_NAV.rent.slice(0, 2),
    ],
  },
...
// Mobile nav uses a flat list with grouping labels
const MOBILE_SECTIONS = [
  { title: 'Main',    items: PUBLIC_NAV.main.slice(0, 3) },
  { title: 'Buy',     items: PUBLIC_NAV.buy },
  { title: 'Rent',    items: PUBLIC_NAV.rent },
  { title: 'Sell',    items: PUBLIC_NAV.sell },
  { title: 'Company', items: PUBLIC_NAV.company },
];
...
const DropdownNavItem: React.FC<DropdownNavItemProps> = ({ group }) => {
  ...
  return (
    ...
    >
      <NavLink
        to={group.path}
        className={({ isActive }) => `public-navbar__link public-navbar__link--has-caret${isActive ? ' active' : ''}`}
        onClick={() => setOpen(false)}
        aria-haspopup="menu"
        aria-expanded={open}
      >
        {group.label}
        <svg className="public-navbar__caret" aria-hidden="true" width="10" height="6" viewBox="0 0 10 6">
        ...
        <div className="public-navbar__dropdown" role="menu">
          {group.children!.map((item) => (
            <Link
              ...
              onClick={() => setOpen(false)}
            >
              <span className="public-navbar__dropdown-icon" aria-hidden="true">{item.icon}</span>
        ...
        <>
          ...
          >
            ...
            <div className="public-navbar__mobile-head">
              <Link to="/" className="public-navbar__mobile-brand" onClick={() => setIsMobileOpen(false)}>
                <img src={logoSrc} alt="White Caves" width={32} height={32} className="public-navbar__brand-logo" />
                  ...
                  <nav aria-label={`${section.title} navigation`}>
                    {section.items.map((item) => (
                      <NavLink
                        key={item.path}
                        to={item.path}
                        className={({ isActive }) => `public-navbar__mobile-link${isActive ? ' active' : ''}`}
                      >
                        <span className="public-navbar__mobile-link-icon" aria-hidden="true">{item.icon}</span>
                        <span>{item.label}</span>
                        <span className="public-navbar__mobile-link-arrow" aria-hidden="true">›</span>
                      </NavLink>
                    ))}
                  </nav>
                </div>
            ...
            <div className="public-navbar__mobile-footer">
              <Link to="/services#sell" className="public-navbar__mobile-list-btn" onClick={() => setIsMobileOpen(false)}>
                📋 List Your Property
              </Link>
              <Link to="/signin" className="public-navbar__mobile-signin" onClick={() => setIsMobileOpen(false)}>
                Sign In
              </Link>
            </div>
          </div>
