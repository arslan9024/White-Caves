...
const AI_ASSISTANTS: AssistantItem[] = getAllAssistants().map((a: any) => ({
  id: a.id,
  name: a.name,
...
const DEPARTMENT_CONFIG: Record<string, DeptConfig> = Object.entries(DEPARTMENTS).reduce((acc, [key, dept]: [string, any]) => {
  acc[key] = { 
    label: dept.label, 
    color: dept.color, 
...
interface CrimsonSidebarEnhancedProps {
  activeTab: string;
...
const CrimsonSidebarEnhanced: FC<CrimsonSidebarEnhancedProps> = ({ 
  activeTab, 
  onTabChange, 
  collapsed = false, 
  onToggleCollapse,
  ...
  const assistantsByDepartment = useMemo(() => {
    const grouped: Record<string, AssistantItem[]> = {};
  ...
  const renderNavItem = (tab: TabItem, showLabel = true) => {
    const Icon = tab.icon;
