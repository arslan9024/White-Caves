...
`;
export const BenchTable = styled.table`
...
export const StatusBadge = styled.span<{ $status: string }>`
  display: inline-flex; padding: 4px 10px; border-radius: 4px; font-size: 0.75rem; font-weight: 700;
  background: ${p => p.$status === 'above' ? '#FEE2E2' : p.$status === 'below' ? '#DCFCE7' : '#FEF3C7'};
  color: ${p => p.$status === 'above' ? '#991B1B' : p.$status === 'below' ? '#166534' : '#92400E'};
`;
