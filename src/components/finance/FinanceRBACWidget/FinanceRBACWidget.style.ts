...
export const MatrixTable = styled.table`
  ...
  td:first-child { text-align: left; font-weight: 600; color: var(--color-334155, #334155); }
  tr:hover { background: var(--color-f8fafc, #F8FAFC); }
`;
export const Badge = styled.span<{ $granted: boolean }>`
  display: inline-flex; align-items: center; justify-content: center; padding: 4px 10px; border-radius: 4px; font-size: 0.75rem; font-weight: 700;
  background: ${p => p.$granted ? 'var(--color-dcfce7, #DCFCE7)' : 'var(--color-fee2e2, #FEE2E2)'};
  color: ${p => p.$granted ? 'var(--color-166534, #166534)' : 'var(--color-991b1b, #991B1B)'};
`;
