    ...
    <Root data-testid="lead-import-wizard">
      ...
      {step === 1 && (
        <>
          ...
          <DropZone onClick={handleFileSelect}>
            ...
            <div style={{ fontWeight: 600, color: '#1e293b' }}>Click to select CSV file</div>
            <div style={{ fontSize: '0.8125rem', color: '#94a3b8', marginTop: '0.25rem' }}>Max 10,000 rows · UTF-8 encoding</div>
          </DropZone>
        </>
      )}
      {step === 2 && (
      ...
      {step === 3 && (
        <SuccessBanner>
          ...
          <div style={{ fontWeight: 700, fontSize: '1rem', color: '#15803d' }}>{rowCount} leads imported successfully!</div>
          <button onClick={handleReset} style={{ marginTop: '1rem', color: '#ef4444', background: 'none', border: 'none', cursor: 'pointer', fontWeight: 600 }}>
            Import another file
          </button>
        </SuccessBanner>
      )}
