...
const InventoryPropertySchema = new mongoose.Schema(
  {
    ...
    sourcingMetrics: {
      ...
      successScore: {
        // 1-10, how quickly/easily did this property close
        type: Number,
        min: 1,
        max: 10,
      },
...
InventoryPropertySchema.statics.queryProperties = async function (filters = {}) {
  ...
  const properties = await this.find(query).sort(sortOptions).skip(skip).limit(limit).lean();
...
InventoryPropertySchema.statics.updateStatus = async function (
  propertyId,
  statusUpdate,
  ...
  if (statusUpdate.marketAvailability)
    property.marketAvailability = statusUpdate.marketAvailability;
...
InventoryPropertySchema.statics.getStatusBreakdown = async function () {
  return this.aggregate([
    ...
    {
      $facet: {
        byConstruction: [{ $group: { _id: '$constructionStage', count: { $sum: 1 } } }],
        byOccupancy: [{ $group: { _id: '$occupancyStatus', count: { $sum: 1 } } }],
        byMarketAvailability: [{ $group: { _id: '$marketAvailability', count: { $sum: 1 } } }],
        byFurnishing: [{ $group: { _id: '$furnishingLevel', count: { $sum: 1 } } }],
        byLegalStatus: [{ $group: { _id: '$legalStatus', count: { $sum: 1 } } }],
