...
const normalizeContract = (row: Record<string, any>): StubContract => ({
  ...
  contractNumber: String(
    row.contractNumber ?? row.reference ?? `WC-${new Date().getFullYear()}-N/A`
  ),
  lessorName: String(
    row.lessorName ??
      row.landlordName ??
      row.metadata?.lessorName ??
      row.parties?.lessor ??
      'Unknown Lessor'
  ),
  tenantName: String(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    if (nextStatus !== undefined) {
      ...
      if (!canTransitionStatus(current.status, nextStatus, contractTransitions)) {
        throw new AppError(
          `Invalid contract status transition: ${current.status} -> ${nextStatus}`,
          400
        );
      }
    }
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    if (tenancyModel?.update && tenancyModel?.findUnique) {
      try {
        ...
        if (existing) {
          const currentStatus: StubTenancyAgreement['status'] = allowedStatus.includes(
            existing.status
          )
            ? existing.status
            : 'draft';
...
const mockAdminSettings = new Map<
  string,
  {
    key: string;
    value: unknown;
  ...
  asyncHandler(async (_req: Request, res: Response) => {
    ...
    if (!systemSetting) {
      ...
      res.json({
        success: true,
        data: settingsMap,
        meta: { count: settings.length, fallback: true },
      });
      return;
    }
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    if (!systemSetting) {
      ...
      if (entries.length > 50)
        throw new AppError('Cannot update more than 50 settings at once', 400);
