...
describe('Transactions Routes — /api/transactions', () => {
  ...
  describe('PATCH /api/transactions/:id', () => {
    ...
    it('returns 200 for non-risky status transition (no KYC check)', async () => {
      mockPrisma.transaction.findUnique.mockResolvedValue({
        id: VALID_MONGO_ID, type: 'lease', amount: 50_000, status: 'draft',
        leadId: VALID_MONGO_ID, notes: null, closingDate: null,
      });
      mockPrisma.transaction.update.mockResolvedValue({
        id: VALID_MONGO_ID, status: 'in_progress', type: 'lease', amount: 50_000,
      });
    ...
    it('returns 200 for sale→in_progress with kyc_verified lead', async () => {
      mockPrisma.transaction.findUnique.mockResolvedValue({
        id: VALID_MONGO_ID, type: 'sale', amount: 800_000, status: 'draft',
        leadId: VALID_MONGO_ID, notes: null, closingDate: null,
      });
      mockPrisma.lead.findUnique.mockResolvedValue({ id: VALID_MONGO_ID, tags: ['kyc_verified'] });
      mockPrisma.transaction.update.mockResolvedValue({
        id: VALID_MONGO_ID, status: 'in_progress', type: 'sale', amount: 800_000,
      });
      const res = await request(createApp('manager'))
        .patch(`/api/transactions/${VALID_MONGO_ID}`)
        .send({ status: 'in_progress' });
