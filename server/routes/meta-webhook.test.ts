...
describe('Meta Webhook Routes', () => {
  ...
  describe('GET /verify', () => {
    it('returns challenge when verification succeeds', async () => {
      ...
      const res = await request(app)
        .get('/api/webhooks/meta/verify')
        .query({ 'hub.mode': 'subscribe', 'hub.challenge': 'challenge_token_123', 'hub.verify_token': 'my_verify_token' });
      expect(res.status).toBe(200);
      expect(res.text).toBe('challenge_token_123');
      expect(mockMetaClient.verifyWebhook).toHaveBeenCalledWith('subscribe', 'challenge_token_123', 'my_verify_token');
    });
    ...
    it('returns 403 when verification fails', async () => {
      ...
      const res = await request(app)
        .get('/api/webhooks/meta/verify')
        .query({ 'hub.mode': 'subscribe', 'hub.challenge': 'c', 'hub.verify_token': 'wrong' });
    ...
    it('returns 500 on unexpected error', async () => {
      mockMetaClient.verifyWebhook.mockImplementation(() => { throw new Error('Crash'); });
      const res = await request(app)
        .get('/api/webhooks/meta/verify')
  ...
  describe('POST / (webhook)', () => {
    it('acknowledges receipt and processes WhatsApp messages', async () => {
      const body = {
        entry: [{ changes: [{ value: { messaging_product: 'whatsapp', metadata: { display_phone_number: '+971501234567', phone_number_id: 'pn_456' }, messages: [{ from: '+971509876543', id: 'wamid_1', timestamp: '1700000000', type: 'text', text: { body: 'Hello' } }] } }] }],
      };
      mockMetaClient.parseWebhookEvent.mockReturnValue(body);
      const res = await request(app).post('/api/webhooks/meta').send(body);
      expect(res.status).toBe(200);
      expect(res.body.success).toBe(true);
      expect(mockMetaClient.parseWebhookEvent).toHaveBeenCalledWith(body);
    });

    it('handles status updates', async () => {
      const body = {
        entry: [{ changes: [{ value: { messaging_product: 'whatsapp', metadata: { display_phone_number: '+971501234567', phone_number_id: 'pn_456' }, statuses: [{ id: 'wamid_s1', status: 'delivered', timestamp: '1700000001', recipient_id: '+971509876543' }] } }] }],
      };
      mockMetaClient.parseWebhookEvent.mockReturnValue(body);
      const res = await request(app).post('/api/webhooks/meta').send(body);
      expect(res.status).toBe(200);
    ...
    it('ignores non-WhatsApp events', async () => {
      const body = { entry: [{ changes: [{ value: { messaging_product: 'instagram', metadata: { display_phone_number: '', phone_number_id: '' } } }] }] };
      mockMetaClient.parseWebhookEvent.mockReturnValue(body);
      const res = await request(app).post('/api/webhooks/meta').send(body);
      expect(res.status).toBe(200);
      expect(res.body.success).toBe(true);
  ...
  describe('POST /send', () => {
    it('sends message with valid fields', async () => {
      const res = await request(app).post('/api/webhooks/meta/send').send({ to: '+971501234567', message: 'Hello', conversationId: 'conv-1' });
      expect(res.status).toBe(200);
      expect(res.body.success).toBe(true);
    ...
    it('returns 500 when client throws', async () => {
      ...
      const res = await request(app).post('/api/webhooks/meta/send').send({ to: '+971501234567', message: 'Hello' });
      expect(res.status).toBe(500);
  ...
  describe('POST /template', () => {
    it('sends template with valid fields', async () => {
      const res = await request(app).post('/api/webhooks/meta/template').send({ to: '+971501234567', template: 'welcome', parameters: ['John'], conversationId: 'conv-2' });
      expect(res.status).toBe(200);
      expect(res.body.data.messageId).toBe('wamid_tpl456');
      expect(res.body.data.template).toBe('welcome');
      expect(mockMetaClient.sendTemplate).toHaveBeenCalledWith('+971501234567', 'welcome', ['John']);
    });

    it('returns 400 when to is missing', async () => {
      const res = await request(app).post('/api/webhooks/meta/template').send({ template: 'welcome' });
      expect(res.status).toBe(400);
      expect(res.body.error).toContain('Missing required fields');
    });
    ...
    it('returns 500 when client throws', async () => {
      ...
      const res = await request(app).post('/api/webhooks/meta/template').send({ to: '+971501234567', template: 'bad' });
      expect(res.status).toBe(500);
  ...
  describe('GET /status', () => {
    it('returns configured status', async () => {
      const orig = { t: process.env.META_ACCESS_TOKEN, b: process.env.META_BUSINESS_ACCOUNT_ID, p: process.env.META_PHONE_NUMBER_ID };
      process.env.META_ACCESS_TOKEN = 'tok'; process.env.META_BUSINESS_ACCOUNT_ID = 'biz'; process.env.META_PHONE_NUMBER_ID = 'pn';
      const res = await request(app).get('/api/webhooks/meta/status');
      expect(res.status).toBe(200);
      expect(res.body.data.configured).toBe(true);
      expect(res.body.data.apiVersion).toBe('v17.0');
      process.env.META_ACCESS_TOKEN = orig.t; process.env.META_BUSINESS_ACCOUNT_ID = orig.b; process.env.META_PHONE_NUMBER_ID = orig.p;
    });

    it('shows not configured when env vars missing', async () => {
      const orig = { t: process.env.META_ACCESS_TOKEN, b: process.env.META_BUSINESS_ACCOUNT_ID, p: process.env.META_PHONE_NUMBER_ID };
      delete process.env.META_ACCESS_TOKEN; delete process.env.META_BUSINESS_ACCOUNT_ID; delete process.env.META_PHONE_NUMBER_ID;
      const res = await request(app).get('/api/webhooks/meta/status');
      expect(res.status).toBe(200);
    ...
    it('returns 500 when getStats throws', async () => {
      mockMetaClient.getStats.mockImplementationOnce(() => { throw new Error('Client error'); });
      const res = await request(app).get('/api/webhooks/meta/status');
      expect(res.status).toBe(500);
  ...
  describe('POST /image', () => {
    it('sends image with valid fields', async () => {
      const res = await request(app).post('/api/webhooks/meta/image').send({ to: '+971501234567', imageUrl: 'https://example.com/photo.jpg', conversationId: 'conv-3' });
      expect(res.status).toBe(200);
      expect(res.body.data.messageId).toBe('wamid_img789');
      expect(res.body.data.channel).toBe('META_API');
      expect(mockMetaClient.sendImage).toHaveBeenCalledWith('+971501234567', 'https://example.com/photo.jpg');
    });

    it('returns 400 when to is missing', async () => {
      const res = await request(app).post('/api/webhooks/meta/image').send({ imageUrl: 'https://example.com/photo.jpg' });
      expect(res.status).toBe(400);
      expect(res.body.error).toContain('Missing required fields');
    });
    ...
    it('returns 500 when client throws', async () => {
      ...
      const res = await request(app).post('/api/webhooks/meta/image').send({ to: '+971501234567', imageUrl: 'https://example.com/photo.jpg' });
      expect(res.status).toBe(500);
