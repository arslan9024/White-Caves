    ...
    mockPrisma: {
      property: {
        ...
        create: fn().mockResolvedValue({
          id: 'prop-1', title: 'Marina Apt', type: 'apartment', status: 'available',
          price: 500000, location: 'Dubai Marina', area: 'Marina',
          bedrooms: 2, bathrooms: 2, sqft: 1200, featured: false,
        }),
        update: fn().mockResolvedValue({
          id: 'prop-1', title: 'Updated Apt', status: 'available',
        }),
        groupBy: fn().mockResolvedValue([]),
        aggregate: fn().mockResolvedValue({
          _sum: { price: 5000000 }, _avg: { price: 500000, sqft: 1200 },
          _min: { price: 200000 }, _max: { price: 2000000 },
        }),
      },
      activity: {
...
describe('Properties Routes — /api/properties', () => {
  ...
  describe('GET /api/properties', () => {
    ...
    it('supports search, status, type filters', async () => {
      ...
      const res = await request(createApp('owner'))
        .get('/api/properties?search=marina&status=available&type=apartment');
      expect(res.status).toBe(200);
    });
    ...
    it('supports bedroom/bathroom filters', async () => {
      ...
      mockPrisma.property.count.mockResolvedValueOnce(0);
      const res = await request(createApp('owner'))
        .get('/api/properties?minBeds=2&minBaths=1');
      expect(res.status).toBe(200);
    });
  ...
  describe('GET /api/properties/:id', () => {
    it('returns property details', async () => {
      mockPrisma.property.findUnique.mockResolvedValueOnce({
        id: VALID_ID, title: 'Marina Apt', status: 'available',
        user: { id: 'user-1', name: 'Agent', email: 'agent@test.com' },
      });
      const res = await request(createApp('agent'))
        .get(`/api/properties/${VALID_ID}`);
      expect(res.status).toBe(200);
  ...
  describe('POST /api/properties', () => {
    it('returns 201 on successful creation', async () => {
      mockPrisma.property.create.mockResolvedValueOnce({
        id: 'prop-new', title: 'New Villa', price: 1000000,
      });
      const res = await request(createApp('owner'))
        .post('/api/properties')
        .send({
          title: 'New Villa', price: 1000000, location: 'Palm Jumeirah',
    ...
    it('returns 403 for finance role', async () => {
      const res = await request(createApp('finance'))
        .post('/api/properties')
        .send({
          title: 'Villa', price: 1000000, location: 'Dubai',
          type: 'villa', status: 'available',
        });
      expect(res.status).toBe(403);
    });
    ...
    it('allows agent to create properties', async () => {
      mockPrisma.property.create.mockResolvedValueOnce({
        id: 'prop-agent', title: 'Agent Listing', price: 500000,
      });
      const res = await request(createApp('agent'))
        .post('/api/properties')
        .send({
          title: 'Agent Listing', price: 500000, location: 'JBR',
    ...
    it('logs activity on creation', async () => {
      mockPrisma.property.create.mockResolvedValueOnce({
        id: 'prop-new', title: 'Test', price: 100000,
      });
      await request(createApp('owner'))
        .post('/api/properties')
        .send({
          title: 'Test', price: 100000, location: 'Dubai',
          type: 'apartment', status: 'available',
        });
  ...
  describe('PATCH /api/properties/:id', () => {
    it('returns 200 on successful update by admin', async () => {
      mockPrisma.property.findUnique.mockResolvedValueOnce({
        id: VALID_ID, title: 'Old', status: 'available', userId: 'other-user',
      });
      mockPrisma.property.update.mockResolvedValueOnce({
        id: VALID_ID, title: 'Updated', status: 'available',
      });
      const res = await request(createApp('owner'))
    ...
    it('allows property owner to update their own property', async () => {
      mockPrisma.property.findUnique.mockResolvedValueOnce({
        id: VALID_ID, title: 'My Villa', status: 'available', userId: 'user-1',
      });
      mockPrisma.property.update.mockResolvedValueOnce({
        id: VALID_ID, title: 'My Updated Villa', status: 'available',
      });
    ...
    it('returns 403 if not admin and not property owner', async () => {
      mockPrisma.property.findUnique.mockResolvedValueOnce({
        id: VALID_ID, title: 'Other Villa', status: 'available', userId: 'other-user',
      });
      const res = await request(createApp('agent', 'user-1'))
        .patch(`/api/properties/${VALID_ID}`)
        .send({ title: 'Hijack' });
    ...
    it('logs status change activity', async () => {
      ...
      mockPrisma.property.update.mockResolvedValueOnce({
        id: VALID_ID, title: 'My Villa', status: 'sold',
      });
      await request(createApp('owner'))
        .patch(`/api/properties/${VALID_ID}`)
        .send({ status: 'sold' });
  ...
  describe('DELETE /api/properties/:id', () => {
    it('returns 200 on successful deletion', async () => {
      mockPrisma.property.findUnique.mockResolvedValueOnce({
        id: VALID_ID, title: 'To Delete', userId: 'user-1',
      });
      const res = await request(createApp('owner'))
        .delete(`/api/properties/${VALID_ID}`);
      expect(res.status).toBe(200);
      expect(res.body.success).toBe(true);
    });
    ...
    it('returns 403 if not admin and not property owner', async () => {
      mockPrisma.property.findUnique.mockResolvedValueOnce({
        id: VALID_ID, title: 'Other Villa', userId: 'other-user',
      });
      const res = await request(createApp('agent', 'user-1'))
        .delete(`/api/properties/${VALID_ID}`);
      expect(res.status).toBe(403);
    ...
    it('cleans up commissions and leads in transaction', async () => {
      mockPrisma.property.findUnique.mockResolvedValueOnce({
        id: VALID_ID, title: 'Clean Up', userId: 'user-1',
      });
      await request(createApp('owner'))
        .delete(`/api/properties/${VALID_ID}`);
