...
if (!_jwtSecret) {
  log.warn('⚠️  JWT_SECRET not set — using dev-only fallback. Never deploy without it.');
}
...
if (!_databaseUrl) {
  log.warn('⚠️  DATABASE_URL not set — Prisma will fail to connect. Set it in .env');
}
