...
describe('messageProcessor', () => {
  ...
  describe('extractEntities', () => {
    it('returns array', () => {
      expect(Array.isArray(extractEntities('I want a 3 bedroom apartment in Amman'))).toBe(true);
    });
    it('handles empty string', () => {
      expect(Array.isArray(extractEntities(''))).toBe(true);
    });
  });
  ...
  describe('calculateLeadScore', () => {
    ...
    it('increases score for high engagement', () => {
      expect(calculateLeadScore({ messageCount: 20, responseRate: 0.9 })).toBeGreaterThanOrEqual(50);
    });
  ...
  describe('analyzeConversationState', () => {
    ...
    it('handles empty messages', () => {
      expect(analyzeConversationState([])).toBeDefined();
    });
  });
});
