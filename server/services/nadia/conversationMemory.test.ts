...
describe("ConversationMemory", () => {
  ...
  describe("updateContext", () => {
    it("adds message", () => { memory.getOrCreateContext("u"); memory.updateContext("u", makeMsg("hi", {conversationId:"u"}), makeIR()); expect(memory.getOrCreateContext("u").messageHistory.length).toBe(1); });
    it("updates intents", () => { memory.getOrCreateContext("u2"); const r = memory.updateContext("u2", makeMsg("v", {conversationId:"u2"}), makeIR("PROPERTY_INQUIRY")); expect(r.recentIntents).toContain("PROPERTY_INQUIRY"); });
  });
