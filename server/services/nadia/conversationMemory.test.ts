...
describe('ConversationMemory', () => {
  ...
  describe('updateContext', () => {
    ...
    it('limits recent intents to 5', () => {
      for (let i = 0; i < 8; i++) mem.updateContext('conv-1', makeMsg('m' + i, 'msg'), makeIR('GREETING'));
      const ctx = mem.getOrCreateContext('conv-1');
  ...
  describe('getContext', () => {
    it('returns undefined for unknown conversation', () => { expect(mem.getContext('nope')).toBeUndefined(); });
    it('returns existing context', () => { mem.getOrCreateContext('conv-1'); expect(mem.getContext('conv-1')).toBeDefined(); });
  });
  ...
  describe('getStats', () => {
    ...
    it('returns totalMessages count', () => {
      mem.updateContext('conv-1', makeMsg('m1', 'hello'), makeIR());
      expect(mem.getStats().totalMessages).toBe(1);
    });
    it('returns averageThemes', () => { expect(mem.getStats()).toHaveProperty('averageThemes'); });
  });
  ...
  describe('isolation', () => {
    it('keeps separate contexts per conversation', () => {
      ...
      expect(mem.getStats().totalConversations).toBe(2);
    });
  });
});
