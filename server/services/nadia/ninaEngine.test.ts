  ...
  describe('processMessage', () => {
    ...
    it('detects complaint with strong text', () => {
      const result = engine.processMessage(
        'I have a complaint and I am unhappy and disappointed about the wrong thing', makeContext()
      );
    ...
    it('returns entities array', () => {
      expect(Array.isArray(engine.processMessage('I need a 3 bedroom villa in Abdoun', makeContext()).entities)).toBe(true);
    });
    it('returns topics array', () => {
      expect(Array.isArray(engine.processMessage('Tell me about the property details', makeContext()).topics)).toBe(true);
    });
    it('has requiresAgentHandoff flag', () => {
      expect(typeof engine.processMessage('hello', makeContext()).requiresAgentHandoff).toBe('boolean');
    });
