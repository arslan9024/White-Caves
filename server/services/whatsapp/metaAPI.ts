...
const log = createLogger('MetaApiClient');
...
export class MetaAPIClient {
  ...
  public async sendMessage(toPhoneNumber: string, messageText: string): Promise<string> {
    try {
      ...
      log.info(`[Meta API] Message sent: ${messageId} to ${toPhoneNumber}`);

      return messageId;
    } catch (error) {
      log.error('[Meta API] Error sending message:', error);
  ...
  public async sendTemplate(
    ...
    try {
      ...
      log.error('[Meta API] Error sending template:', error);
      throw this.handleError(error);
    }
  }
  ...
  public async sendImage(toPhoneNumber: string, imageUrl: string): Promise<string> {
    try {
      ...
      log.error('[Meta API] Error sending image:', error);
      throw this.handleError(error);
    }
  }
  ...
  public async sendDocument(toPhoneNumber: string, documentUrl: string, filename?: string): Promise<string> {
    try {
      ...
      log.error('[Meta API] Error sending document:', error);
      throw this.handleError(error);
    }
  }
  ...
  public async getMessageStatus(messageId: string): Promise<MessageStatus> {
    try {
      ...
      log.error('[Meta API] Error getting message status:', error);
      throw this.handleError(error);
    }
  }
  ...
  public async uploadMedia(fileBuffer: Buffer, mimeType: string, filename: string): Promise<MediaUploadResponse> {
    try {
      ...
      log.error('[Meta API] Error uploading media:', error);
      throw this.handleError(error);
    }
  }
  ...
  public async downloadMedia(mediaId: string): Promise<Buffer> {
    try {
      ...
      log.error('[Meta API] Error downloading media:', error);
      throw this.handleError(error);
    }
  }
  ...
  public verifyWebhook(mode: string, challenge: string, verifyToken: string): string | null {
    if (mode === 'subscribe' && verifyToken === this.config.webhookVerifyToken) {
      log.info('[Meta API] Webhook verified');
      return challenge;
    }
    log.warn('[Meta API] Webhook verification failed');
    return null;
  }
  ...
  public async getAccountInfo(): Promise<any> {
    try {
      ...
      log.error('[Meta API] Error getting account info:', error);
      throw this.handleError(error);
    }
  }
  ...
  public async getPhoneNumberInfo(): Promise<any> {
    try {
      ...
      log.error('[Meta API] Error getting phone number info:', error);
      throw this.handleError(error);
    }
  }
