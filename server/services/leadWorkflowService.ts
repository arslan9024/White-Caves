...
function getTaskColumn(lead: LeadTaskLead, now: Date): TaskColumnKey {
  ...
  if (
    lead.createdAt.toDateString() === now.toDateString() ||
    ['qualified', 'viewing'].includes(lead.status)
  ) {
    return 'today';
  }
  return 'upcoming';
...
export function buildLeadTaskCockpit(leads: LeadTaskLead[], now = new Date()) {
  ...
  const sorted = leads
    ...
    .sort(
      (a, b) => b.priorityScore - a.priorityScore || b.score - a.score || a.hoursOpen - b.hoursOpen
    )
    ...
    for (const userId of recipientIds) {
      await notifications.pushToUser({
        ...
        metadata: {
          ...
          escalationLevel: hoursOpen >= LEAD_MANAGER_ESCALATION_HOURS ? 'manager' : 'assignee',
        },
      });
    }
