  ...
  it('sends assignee and manager SLA escalations for severely overdue leads', async () => {
    ...
    const result = await runLeadSlaEscalationTick({
      ...
      prismaClient: {
          ...
          findMany: vi.fn().mockResolvedValue([
            {
              ...
              name: 'Overdue Lead',
              status: 'new',
              createdAt: new Date(`2026-05-27T${String(12 - LEAD_SLA_HOURS - 5).padStart(2, '0')}:00:00.000Z`),
              assignedToId: 'agent-1',
        ...
        user: {
          findMany: vi.fn().mockResolvedValue([{ id: 'manager-1', name: 'Manager One', email: 'manager@wc.ae' }]),
        },
        activity: {
