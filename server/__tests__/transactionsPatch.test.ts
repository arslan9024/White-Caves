...
vi.mock('../middleware/errorHandler', () => ({
  ...
  asyncHandler: (fn: unknown) => (req: express.Request, res: express.Response, next: express.NextFunction) =>
    Promise.resolve((fn as (req: express.Request, res: express.Response, next: express.NextFunction) => unknown)(req, res, next)).catch(next),
}));
vi.mock('../middleware/rbac', () => ({
  requirePermission: () => (_req: express.Request, _res: express.Response, next: express.NextFunction) => next(),
  requireRole: () => (_req: express.Request, _res: express.Response, next: express.NextFunction) => next(),
  requireMinRole: () => (_req: express.Request, _res: express.Response, next: express.NextFunction) => next(),
  scopeToOwn: () => (_req: express.Request, _res: express.Response, next: express.NextFunction) => next(),
...
describe('PATCH /api/transactions/:id — KYC gate (P0-013)', () => {
  ...
  it('returns 403 for insufficient role', async () => {
    mockPrisma.transaction.findUnique.mockResolvedValue({ id: VALID_ID, type: 'sale', amount: 100, status: 'draft', leadId: VALID_ID });
  ...
  it('returns 200 for sale→in_progress with kyc_verified lead', async () => {
    mockPrisma.transaction.findUnique.mockResolvedValue({
      id: VALID_ID, type: 'sale', amount: 800_000, status: 'draft', leadId: VALID_ID,
    });
  ...
  it('returns 403 for amount≥500k non-sale→completed without KYC', async () => {
    mockPrisma.transaction.findUnique.mockResolvedValue({
      id: VALID_ID, type: 'deposit', amount: 600_000, status: 'draft', leadId: VALID_ID,
    });
