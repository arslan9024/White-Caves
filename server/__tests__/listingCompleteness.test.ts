...
const fullyCompleteProperty = {
  ...
  description:         'This stunning penthouse offers panoramic views of Dubai Marina with world-class amenities.',
  price:               5_500_000,
  type:                'penthouse',
  status:              'available',
...
describe('GET /api/properties/:id/completeness', () => {
  ...
  it('returns 200 with score, passed, failed, totalCriteria', async () => {
    ...
    const res = await request(createApp()).get(`/api/properties/${fullyCompleteProperty.id}/completeness`);
  ...
  it('returns score < 50 for a minimal (mostly empty) listing', async () => {
    ...
    const res = await request(createApp()).get(`/api/properties/${minimalProperty.id}/completeness`);
  ...
  it('identifies images as failed when there are fewer than 3 photos', async () => {
    mockPrisma.property.findUnique.mockResolvedValue({
      ...fullyCompleteProperty, id: 'prop-few-imgs', images: ['img1.jpg'],
    });
...
describe('GET /api/properties/completeness-summary', () => {
  ...
  it('band counts sum to the total property count', async () => {
    ...
    const bandSum = Object.values(bands as Record<string, number>)
      .reduce((s, v) => s + v, 0);
